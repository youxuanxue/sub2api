...
| `.github/workflows/main-ancestry-guard.yml` | any PR with `base.ref == 'main'` | Fails the PR if `git merge-base --is-ancestor PR_base PR_head` exits non-zero — i.e. the PR was opened from an orphan branch (or otherwise rewritten history not descending from main). Catches the exact failure mode that orphan-reset main in PR #307: GitHub's "Squash and merge" would otherwise rewrite main into the orphan tree, severing every commit (including release tags) that was on main but not on the PR head. Also blocks modifying `.main-ancestry-anchor` unless a PR commit message carries the literal marker `main-ancestry-anchor-advance` (see anchor advancement procedure below). |
...
The Layer A workflow recognizes the marker and allows the anchor change for that single PR.
