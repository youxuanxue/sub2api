...
| `scripts/checks/upstream-override-marker.py` (via `scripts/preflight.sh`) | every PR (CI preflight + local pre-commit) | When the PR diff touches any upstream-shaped path (handlers/services/views/… excluding `*_tk_*.go` / `*.tk.ts` / `*_test.go` / TK-only subpackages), require **either** an updated `scripts/sentinels/*.json` **or** a commit-message marker: `upstream-touch-guarded` / `upstream-touch-trivial` / `upstream-merge` / `no-upstream-touch`. Forced acknowledgement before merge, like `no-web-impact`. |
