...
适用于本仓库（TokenKey fork of sub2api）。五条**完全平行**的写入面，都由 `ops/anthropic/manage-anthropic-config.py` 编排，**都遵守同一组确定性纪律**（详 §0）：
...
| 单元测试覆盖 plan 渲染 / apply 路由 | 机械 | `ops/anthropic/test_manage_anthropic_config_{plan,stub_pool,concurrency_mirror,group_claude_code_only,operator_balance}.py`（preflight 跑） |
| edge operator balance 自动检查 | 机械 | `check` 内嵌 `operator_balance` 段（snapshot v6+ 或 live SSM）；`plan-edge-operator-balance` + apply 后 DEL `billing:balance:1` |
| §0 「数据值散落多处」「列号读取」「跨账号脑补」防御 | 判断 + 机械 | 见 §0 表格：每条「风险」都有对应「固化机制」 |
| 写入面边界（B 写 credentials 子键、(C) 不动 credentials、(D) 只写 claude_code_only、admin UI 负责 group.rpm_limit） | 判断 | prompt（产品边界 + admin UI 分工） |
...
| (E) edge operator balance | `anthropic-edge-operator-balance-baselines.json` | `edge_operator_balance` | 每个 deployable edge：`users.id=1.balance` 若 `< min_balance_threshold`（默认 100）→ 设为 `default_balance`（默认 9999999）；apply 后失效 Redis `billing:balance:1` |
...
# Stage 2 — Check：OAuth guard + edge users.id=1 余额门槛（<100 报 violation）
...
python3 $MGR plan-concurrency-mirror \
  --snapshot $JOBDIR/snap.json --out $JOBDIR/plan.json
#   3e) 所有 anthropic group 打开「仅 Claude Code 客户端」（claude_code_only=true）：
python3 $MGR plan-group-claude-code-only \
  --snapshot $JOBDIR/snap.json --out $JOBDIR/plan-group-cc.json
#   3f) edge admin（users.id=1）余额低于门槛则顶回 default_balance：
python3 $MGR plan-edge-operator-balance \
  --snapshot $JOBDIR/snap.json --out $JOBDIR/plan-balance.json
...
| check | snap.json | 每个 edge 跑 `check-edge-oauth-stability.py` + `operator_balance` 段（live 或 snapshot v6） | 0 ok / 1 violation / 2 error |
...
| plan-edge-operator-balance | snap.json | `plan.json`：每个 deployable edge 上 `users.id=1.balance < threshold` 一个 `kind=edge_operator_balance` action；全达标则 `noop=true` | 0 / 2 |
...
### 写入面 (D) 配方：anthropic group「仅 Claude Code 客户端」

**UI 语义（勿反）**：admin「Claude Code 客户端限制」**开关 ON** → `groups.claude_code_only=true` → **仅**官方 Claude Code 客户端可走该分组；非 CC 流量命中 `ErrClaudeCodeOnly` 或走 `fallback_group_id`（须在 admin UI 为各组配置降级分组）。**开关 OFF** 时 UI 显示「允许所有客户端」→ `claude_code_only=false`——**不是**本写入面目标。

**触发场景**：运维要求**所有** `platform=anthropic` 分组统一打开 CC 限制（prod + 各 deployable edge）。

**真值源**：`deploy/aws/stage0/anthropic-group-claude-code-baselines.json`（`policy.claude_code_only: true`）。**每个目标库独立**（edge uk1/us1/… 与 prod 各有一份 `groups` 表），plan 对每个仍有 `claude_code_only!=true` 的 anthropic group 的 target 发一个 action。

```bash
python3 $MGR snapshot --out $JOBDIR/snap.json   # v5+ 含 edges.*.anthropic_groups + prod.anthropic_groups
python3 $MGR plan-group-claude-code-only \
  --snapshot $JOBDIR/snap.json --out $JOBDIR/plan.json
python3 $MGR apply  --plan $JOBDIR/plan.json --confirm yes-apply-anthropic-config-cascade
python3 $MGR verify --plan $JOBDIR/plan.json
...
`noop=true` 表示该 target 上所有 anthropic group 已是 `claude_code_only=true`。本面**不改** `fallback_group_id`（降级路径仍走 admin UI）；**不改** `group.rpm_limit`。

### 写入面 (E) 配方：edge admin 用户余额门槛

**触发场景**：edge 控制面 `users.id=1`（默认 admin）余额被消耗到 `< 100`，admin API / 网关计费预检会拒绝请求。

**策略**（`anthropic-edge-operator-balance-baselines.json`）：`min_balance_threshold=100`，`default_balance=9999999`。仅 **deployable edge**；**不写 prod**。

```bash
python3 $MGR snapshot --out $JOBDIR/snap.json   # v6+ 含 operator_user_balance
python3 $MGR check --snapshot $JOBDIR/snap.json # operator_balance.violation_count 应为 0 或列出待修 edge
python3 $MGR plan-edge-operator-balance \
  --snapshot $JOBDIR/snap.json --out $JOBDIR/plan-balance.json
python3 $MGR apply  --plan $JOBDIR/plan-balance.json --confirm yes-apply-anthropic-config-cascade
python3 $MGR verify --plan $JOBDIR/plan-balance.json
```

`check` 无 snapshot 时会对各 edge  live 查余额；有 snapshot 时必须 **v6+**（含 `operator_user_balance`）。apply 在 DB `UPDATE` 成功后 **DEL** Redis `billing:balance:1`。
...
本流水线**五条**写入面（详 §0 表格）：
- (A) edge OAuth tier baseline `extra.*`（含 `enable_tls_fingerprint` / `tls_fingerprint_profile_id` 与同名的 `tls_fingerprint_profiles` upsert）+ `concurrency` + `priority` + `stability_tier` + 同事务 `users.id=1.concurrency` Σ 对齐（TLS canonical 名称见上文 **Anthropic OAuth：TLS fingerprint 模板**）
...
- (D) anthropic group `claude_code_only=true`（仅 Claude Code 客户端）——每个 deployable edge + prod 各库的全部 `platform=anthropic` group
- (E) edge `users.id=1.balance` 门槛（低于 100 → 顶回 9999999）——仅 deployable edge
...
| edge / prod 其他 `group` 字段（name / fallback / model_routing / …） | admin UI |
...
- 写入面 (D) group Claude Code only baseline：`deploy/aws/stage0/anthropic-group-claude-code-baselines.json`（`policy.claude_code_only: true`；orchestrator 在 plan-group-claude-code-only 中读它）
- 写入面 (E) edge operator balance baseline：`deploy/aws/stage0/anthropic-edge-operator-balance-baselines.json`（`min_balance_threshold` + `default_balance`；orchestrator 在 plan-edge-operator-balance / check 中读它）
