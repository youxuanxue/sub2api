...
prod 上**自持 edge 主路径**类型的 anthropic stub（`credentials.base_url` 匹配 `https://api-<edge_id>.tokenkey.dev` 模式）必须**镜像**其上游 edge 的容量。镜像分两层：账号级（R1）与分组级（R3），由同一个 guard 统一校验。

### R1 — 账号级镜像（self-edge stub.concurrency）

- `accounts.concurrency` ≡ edge OAuth.concurrency — prod 端 in-flight 与 edge 实际容量对齐，溢出立刻 fallback 而不是堆积成上游 429。

### R3 — 分组级镜像（self-edge group.rpm_limit）

对**每一个** prod anthropic group `g`（无论 oauth-only / stub-only / 混合）：

- 收集 `g` 内**所有 self-edge stub**（成员账号中 `type='apikey'` 且 `base_url` 匹配 `https://api-<edge_id>.tokenkey.dev`）。
- 若 self-edge 集合为空 ⇒ `g` 与任何 edge 无 prod→edge passthrough 拓扑 ⇒ skip R3。
- 否则：对每个独立 `edge_id`，查它上的 anthropic `default` group 的 `rpm_limit`（stub 默认 passthrough 到 edge `default`，详见 §"prod 控制面：anthropic stub 双绑规则"中"edge 数据库内部"一条）。
- 计算 `max_edge_rpm_limit = max over distinct edges`，其中 edge 侧的 `0` / `NULL` 解释为 `+∞`（无限）。
- 约束：`g.rpm_limit == 0`（或 `NULL`，prod 端无限）**或** `g.rpm_limit >= max_edge_rpm_limit`。
- 违规含义：prod 比 edge 更紧，会替 edge 提前 429 / 503，破坏 passthrough 拓扑意图；事故 2026-05-18 01:38-01:42 UTC 即此模式。

`0` 在两端的语义不对称设计：
- prod 端 `0` ⇒ 不限速，恒满足约束。
- edge 端 `0` ⇒ 容量无限，则 prod 唯一能"不更紧"的写法是同样 `0`；否则 guard 报错并要求把 prod 改为 `0`。
...
`base_url` 不匹配 self-edge 模式（例如 `https://agent.tokensea.ai`）的 stub 享有独立容量：**不参与 R1 镜像 check**，**也不计入 R3 的 self-edge fan-out 集合**，但仍需满足共同 baseline。
...
- exit 0 — 所有 stub 通过 R1 + baseline，且所有 self-edge-bearing group 通过 R3
- exit 1 — 至少一处违规（R1 / baseline / R3 任一），**修复后才能 apply**
- exit 2 — SSM / schema / target 解析失败

报告输出按 `--- account-level (R1) ---` 与 `--- group-level (R3) ---` 两段呈现；`stub_violation_count` 与 `group_violation_count` 在 JSON 模式下分项汇总。

### 规则的本质

**prod 是 router、edge 是 worker**。Router 的容量（每账号 in-flight、每组 RPM）必须 ≥ worker 的真实容量；任何缩紧只把可服务的请求挡在 prod 入口，浪费 edge quota 并把伪造的 429/503 推给客户端。Mirror 把这层语义代码化，让任何 edge tier 变更（如 H1 升降）能自动被 prod stub guard 捕获。
