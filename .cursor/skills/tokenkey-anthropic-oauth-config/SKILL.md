...
description: >-
  ...
  **四条写入面**，都由同一个脚本 ops/anthropic/manage-anthropic-config.py 编排，且都
...
适用于本仓库（TokenKey fork of sub2api）。四条**完全平行**的写入面，都由 `ops/anthropic/manage-anthropic-config.py` 编排，**都遵守同一组确定性纪律**（详 §0）：
...
| 单元测试覆盖 plan 渲染 / apply 路由 | 机械 | `ops/anthropic/test_manage_anthropic_config_{plan,stub_pool,concurrency_mirror,group_claude_code_only}.py`（preflight 跑） |
