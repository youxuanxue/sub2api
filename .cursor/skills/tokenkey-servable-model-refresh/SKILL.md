...
### OpenAI / GPT 新型号：prod floor 拦截不等于 edge OAuth 可服务

OpenAI catalog refresh 的 `OPENAI_*` 默认只证明**当前 prod group_id=2 普通服务面**是否可服务。
对尚未进入 Go allowlist / account `model_mapping` floor 的新型号，prod 可能在调度前返回
`400 Unsupported model: <id>`（`ops_error_logs.account_id=null`、无 upstream event）；这只是
prod floor/model_mapping 未放行，不能证明 edge OAuth 上游账号支持该型号。不要因为已补价或
overlay 存在就直接把它加进 allowlist/runtime。

需要区分时，用 `tokenkey-account-model-probe` 对 deployable edge 的 OpenAI OAuth 单账号实测：

```bash
# control：先证明同一 edge OAuth 账号能服务已知型号
bash ops/observability/run-probe.sh --target edge:us4 \
  ...
  --env ACCOUNT_ID=7 --env MODEL=gpt-5.5 --env ENDPOINT=responses --timeout-seconds 180

# 目标型号：必须看到 verdict=servable 才能进 catalog/Menu 或 runtime mapping
bash ops/observability/run-probe.sh --target edge:us4 \
  --script ops/stage0/probe_account_model.sh \
  --with ops/pricing/probe_reserved_resources.sh \
  --env ACCOUNT_ID=7 --env MODEL=gpt-5.6 --env ENDPOINT=responses --timeout-seconds 180
```
...
所以不能通过清开 prod model_mapping/floor 单独上架，当前应保持不展示。
...
- **verdict 语义**：200=servable（留）；400/404+retired/not-found/"not supported when using
  ...
  （探测设置坏了，不是模型信号——先修 key/形状再重跑）。OpenAI prod 普通探测里的
  `Unsupported model: <id>` 可能是本地 allowlist/model_mapping floor 拦截；要用
  `tokenkey-account-model-probe` 单账号 edge OAuth 复核上游能力后再决定是否展示。
