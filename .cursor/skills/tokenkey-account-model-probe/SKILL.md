...
- `X-Request-ID` is **server-only** as storage identity. Exact evidence lookup
  uses the **response** `X-Request-ID` (also `probe.server_request_id` /
  `response.x_request_id` in `probe_account_model.sh` output). Never assume a
  request-header value lands in `usage_logs.request_id` or QA blob keys.
- Client-chosen markers prefer inbound `X-Client-Request-ID` →
  `client_request_id`. If a caller only sends legacy `X-Request-ID`, that value
  is kept as `client_request_id` (not storage `request_id`) so a lost response
  is still searchable in logs/ops. Prefer sending `X-Client-Request-ID`
  explicitly; the account-model probe already does.
