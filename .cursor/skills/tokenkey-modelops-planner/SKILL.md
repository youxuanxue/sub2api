...
**CatalogPolicy（官方别名）**：对已支持的平台与 newapi channel，若上游官方 model page
（或 `tk_served_models.json` 策展行）声明了 model id/alias，且能解析到价格 owner、没有
structurally-gone 证据，则可进入公开 catalog/menu（不只列 stable bare id）。仅退休重定向、
无官方 SKU 的兼容键保持 priced-only。请求能否交付仍看 RequestPlan 与 RuntimeReadiness。
详见 `docs/global/agent-reference.md` § Model serving SSOT。
