...
> **实测结论（2026-06-26，edge-us6 真账号 kiro-us6-real）**：`*.kiro.dev` 网关与 legacy
> `q`/`codewhisperer.us-east-1.amazonaws.com` **协议同构**——`runtime.us-east-1.kiro.dev/generateAssistantResponse`
> 用 TK 完整 on-wire 形态（`X-Amz-Target=AmazonCodeWhispererStreamingService.GenerateAssistantResponse`、
> KiroPayload body、aws-sdk-js UA）返回 200 + 真实 assistant event-stream；`management.us-east-1.kiro.dev/ListAvailableProfiles`
> 返回与 legacy **相同的 profileArn**。故迁移 = 安全的换 host（legacy 仍 LIVE，暂不切）。详见
