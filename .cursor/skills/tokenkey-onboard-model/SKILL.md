...
- **真判断（留给人/agent）**：① 官方价是多少、有没有阶梯/思考双档（**禁臆造**，必须查官方页带 `source`
  URL+抓取日）；② `inconclusive`（429/502/503）是"该组真没这账号"还是"模型本身不可用"；③ 该不该清空/
...
# 200=servable（留）；400/404+not-found/retired=unsupported；429/502/503=inconclusive；401/403=auth_error
```

> **注**：dashscope/qwen probe 族（`DASHSCOPE_CHAT_MODELS` → group 18 的 api_key → `/v1/chat/completions`，
> 思考档加 `enable_thinking:true`）按 `ops/pricing/probe-servable-models.sh` 既有族（gemini/ark）的形状新增；
> 取 key 复用 `GEMINI_GROUP_NAME` 那条 `api_keys.group_id→groups.id` join（group 名传 `Qwen`）。**新增 probe
> 族属本 skill 的 column-3 配套**，与本 manifest/guard 解耦——guard 不依赖 probe 跑过，只校验仓库内三方一致。
...
发版/apply 后用 §1 的 probe 族对 model_id 发**思考 + 非思考各一条**真实请求，确认都回 200（"定价就绪 ≠
prod 可服务"，见 memory `priced_not_equal_servable_verify_livefire`）。

### 6) 两档计费核对 + 话术

- 拉一条真实 usage_log，核对：非思考请求按 `output_cost_per_token` 计、思考请求按
  `thinking_output_cost_per_token` 计（dense Qwen3 默认思考档）；cache_read 命中按 `cache_read_input_token_cost`。
- 客户话术：模型名、可用 endpoint（`/v1/chat/completions`）、计费档位（输入/输出/思考输出/缓存）、
  是否默认思考。
...
- **A3 SERVED_ON ⇒ MIGRATION**（硬，**#812 捕获方向**）：每个 `served_on` 账号 A，model_id 须在**某条
  `UPDATE` 语句**内与 `id = A` 同时出现。**语句级 + 去注释**作用域（不是文件级）：因为 SQL 把
  model_mapping jsonb 写在 SET 子句、`WHERE id = N` 在其后，且 tk_022/tk_027 一文件改两账号——按 `;` 切语句、
  剥 `-- 注释`、要求 model_id 双引号字面量与 `id=A` 在同一 `UPDATE` 内共现，既防跨账号串味又防注释假阳。
