...
- **顺序做完**：`release-bump-and-tag.sh` → **watch 到 release 成功** → 根据 `target` dispatch 对应 deploy workflow → **deploy-stage0 部署内嵌 registry 并完成 smoke** → **Anthropic OAuth snapshot/check + Account model_mapping check（默认）** → **再做本地/日志验收 / followup**。不要在 workflow 绿灯后就结束会话。
...
`deploy-stage0.yml` 部署的 tagged binary 已内嵌同一 tag 的
`backend/internal/service/tk_pricing_overlay.json`；价格与代码作为一个 artifact
激活，rollback 也同时恢复两者。不存在独立 pricing settings 同步步骤；完整 smoke
直接验证新实例实际加载的 registry。
