...
| Edge dispatch 路由（唯一 deployable owner） | 机械 | `scripts/stage0/resolve-edge-deploy-route.py --edge-id <id> --json` |
...
| `target=edge-<edge_id>` | 默认 tag 已存在：用 **`bash scripts/stage0/dispatch-edge-deploy.sh`**（跟随唯一 deployable owner）→ watch → 按 phase 验收 smoke。`operation=smoke` 只 smoke；`operation=rollback` 用 `previous_tag`。不要手选 workflow 或手填 confirm token。 |
...
- **顺序做完**：`release-bump-and-tag.sh` → **watch 到 release 成功** → 根据 `target` dispatch 对应 deploy workflow → **deploy-stage0 只读审计 pricing registry runtime 并完成 smoke** → **Anthropic OAuth snapshot/check + Account model_mapping check（默认）** → **再做本地/日志验收 / followup**。价格 snapshot 不随镜像部署或回滚改写。
...
`deploy-stage0.yml` 会在 SSM 换镜像且 external health 通过后，只读审计 protected-main
registry 与 prod runtime snapshot 是否一致，再继续完整 smoke。审计为 advisory，镜像部署和
回滚都不得写价格 setting，避免旧 tag 覆盖较新的热价格。价格变更由 registry PR 合并后
`pricing-registry-publish.yml` 独立发布，不从工作站手工执行 `sync-runtime`。
...
以 `resolve-edge-target.py --list-deployable` 为准。**禁止**手选 workflow——一律走 dispatch 脚本，由它根据唯一 owner 路由：
...
2. 读取 deployable 矩阵并**机械化选 canary**：
   ...
   # 只包含唯一 active owner；migration candidate 不参与普通 rollout
   ```
...
**prod release 默认下一步**：prod smoke 在 CI log 确认 `tk_post_deploy_smoke: OK` 后，立即跑 §「发版后 Anthropic OAuth 配置检查」（除非 `anthropic_config_check=false`）和 §「发版后 Account model_mapping 配置检查」（除非 `account_model_mapping_check=false`）。后者是 yellow drift 诊断，不改变 deploy/smoke verdict。
...
`deploy-stage0.yml` 的 generic deploy / rollback 不依赖 live `model_mapping` convergence，也不要求 previous tag 含 account-mapping Go helper。发生回归时直接 rollback；不得先跑或等待 `release-gate` / `check-accounts`。
...
**做什么 / 不做什么**：本段是 deploy/smoke 之后的**只读 yellow 诊断**——默认仅 prod 经 SSM 读 active 账号和 Antigravity active groups，使用 `backend/cmd/account-model-mapping` 导出的 Go SSOT floor 与 forbidden policy metadata 生成 diff。Python 工具不维护 Grok alias、Kiro 模型、Antigravity live Claude 等“应包含”清单；正向 mapping 与禁止项都由 Go owner 派生。`check-accounts` 检查受管平台账号显式 `credentials.model_mapping`、target、forbidden key/prefix 与 Antigravity group scopes 的严格收敛。edge 账号 `model_mapping` 保持空（全路由），默认不检查；需显式 `--include-edges` 才查 deployable edges。不写任何库；runtime setting 只做合法性校验并作为 desired layer 参与 diff。服务进程不会在启动、`settings_updated` fan-out 或周期 tick 中批量覆盖账号配置；写入必须走 `/tokenkey-modelops-planner` 分支 D 的显式 `apply-accounts --confirm yes-apply-account-model-mapping`。generic deploy 与 rollback 均不调用或依赖本检查。
...
| rc | 摘要 verdict | 动作 |
|---|---|---|
| 0 | **green** — 所有受管平台账号 `model_mapping` 与 Go SSOT floor/policy metadata 一致 | 无需动作 |
| 1 | **yellow** — 有账号/Antigravity group/runtime setting 不满足 SSOT diff | 不 rollback 镜像；审 `post-release-account-model-mapping-check.json`，确认后走 `/tokenkey-modelops-planner` 分支 D 的 `apply-accounts --confirm yes-apply-account-model-mapping` |
...
next: none | 审 diff 后 apply-accounts / 修 runtime JSON / 补 OIDC 后重跑
```
...
- **Account model_mapping 配置检查**：§「发版后 Account model_mapping 配置检查」固定块；violation 时列 `post-release-account-model-mapping-check.json` 里的 target / kind / account 摘要，并明确它是 yellow、不会触发 rollback。
...
- `scripts/stage0/dispatch-edge-deploy.sh` — 单一 Edge deploy dispatch（默认跟随唯一 deployable owner）。
...
- `.github/workflows/deploy-edge-lightsail-stage0.yml` / `deploy-edge-stage0.yml` — Lightsail / EC2 Edge deploy。
