...
9. **只读配置检查（默认，在 followup 之前）**：Account model_mapping 已由 `deploy-stage0.yml` 在 SSM 换镜像前执行 §「发版前 Account model_mapping SSOT 硬门禁」（红灯则 prod 不会部署）；部署成功后只需跑 §「发版后 Anthropic OAuth 配置检查」。Anthropic violation 不触发 rollback，写入 rollout 摘要为 **yellow**，并指向 `/tokenkey-anthropic-oauth-config`。
...
**prod release 默认下一步**：prod smoke 在 CI log 确认 `tk_post_deploy_smoke: OK` 后，立即跑 §「发版后 Anthropic OAuth 配置检查」（除非 `anthropic_config_check=false`）。Account model_mapping 已在 deploy-stage0 换镜像前硬门禁完成；失败时该 workflow 不会进入 deploy/smoke。
...
## 发版前 Account model_mapping SSOT 硬门禁
...
python3 ops/pricing/manage-account-model-mapping-runtime.py release-gate --json \
  | tee "$JOBDIR/pre-deploy-account-model-mapping-gate.json" >/dev/null
MODEL_MAPPING_RC=${PIPESTATUS[0]}
```

**exit 语义**（workflow gate）：
...
```text
=== Pre-deploy Account model_mapping release gate (${NEW_TAG}) ===
result: OK | VIOLATION rc=1 | ERROR rc=2 → $JOBDIR/pre-deploy-account-model-mapping-gate.json
violation_count: <from JSON .violation_count>
error_count: <from JSON .error_count>
next: none | 审 diff 后 apply-accounts / 修 runtime JSON / 补 OIDC 后重跑 deploy
```
...
| account-model-mapping-gate | deploy-stage0 → manage-account-model-mapping-runtime.py release-gate | — | X.Y.Z | gate OK / gate failed | prod 显式 model_mapping（只读；默认不含 edge） |
...
- **Account model_mapping gate**：§「发版前 Account model_mapping SSOT 硬门禁」固定块；如果 gate fail，prod deploy 不应继续，先审 target / kind / account diff 后走 `/tokenkey-modelops-planner` 分支 D。
