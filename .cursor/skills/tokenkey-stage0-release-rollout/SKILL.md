...
Legacy retained-request protocol filter: `ops/stage0/prod_replay_manifest.py` and `ops/stage0/prod-replay-capabilities.json` (explicit `operation=replay` only).
Gateway capability owner: `ops/stage0/gateway_capability_matrix.py` + `gateway-capability-matrix.json`; `scripts/stage0/check-gateway-capabilities.sh` plans/reports offline after release. `post_release_replay_check.py run` is an explicit, bounded isolated check using dedicated test-key bindings, never a deploy gate or cutover approval. The legacy retained-capture experiment remains optional; only explicit replay continuation consumes its receipt. See `docs/approved/prod-replay-capability-matrix.md`.
