...
{"environment_ids":[$ENV_ID],"state":"approved","comment":"canary <edge> infra smoke OK (run <canary run id>)"}
EOF
...
7. **其余 deployable Edge（单命令，fail-stop）**：
   ...
   ```bash
   bash scripts/stage0/rollout-edges.sh --tag "$TARGET_TAG" --skip <canary_edge>
...
- `scripts/stage0/rollout-edges.sh` — 其余 Edge 顺序 rollout（fail-stop + smoke 标记验收）。
- `scripts/stage0/dispatch-edge-deploy.sh` — 单一 Edge deploy dispatch（edges 均为 Lightsail）。
- `ops/observability/probe-post-release-tick.sh` — 发版后 tick 探针（HOOK_PATTERNS 计数 + 流量/5xx/panic）。
