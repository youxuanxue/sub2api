...
`gateway_capability_host.py` 直连正常蓝绿候选内部地址，使用现有 `TK_FULLTEST_KEY`
（可用 `--test-key-name` 指定已有测试 universal key），执行工具续轮/媒体语义、串行限速和 usage 归因。
不创建额外网关/PostgreSQL/Redis，不复制数据库，不创建或重绑测试身份，不设 replay 专属 load/PSI 门槛。
请求正常计费和记录 usage；SQL 查询只读。账号由正常 universal 路由选择，结果记录实际账号及
是否匹配计划账号类。能力不支持、真实错误和未执行项均保留逐条原因，不删分母。
...
测试 key usage 与线上指纹对比；禁止自动继续 prod deploy、smoke 或 edge rollout。
