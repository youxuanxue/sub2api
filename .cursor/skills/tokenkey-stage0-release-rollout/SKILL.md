...
5. **其余 deployable Edge bounded-parallel rollout**：prod full smoke 与 canary main-via-edge 都绿后，剩余 Edge 只做 **upgrade（infra smoke）**，可按小批次并发；批内任一失败即不再 dispatch 下一批。**main-via-edge 只在 canary 做一次**（见 `target=all` 执行顺序）。
