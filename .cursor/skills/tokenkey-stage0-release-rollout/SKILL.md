...
| Edge smoke 分阶段（infra / edge-native-oauth / main-via-edge / full） | 机械 | `ops/stage0/edge_post_deploy_smoke.sh` + workflow `smoke_phase`；upgrade 默认 **full**（infra + 容器内 per-account OAuth 拟真 `probe_account_model`）；`main-via-edge` 为可选 prod 中转链路 |
...
| `target=all` | release 一次 → canary **upgrade (full)** → prod deploy（CI smoke）→ 可选 canary **main-via-edge** → 其余 Edge **upgrade (full)** → followup → **默认** Anthropic OAuth snapshot/check。 |
...
2. **Edge canary：deployable 矩阵第一个 Edge upgrade + full smoke**：验证镜像启动、infra 门禁，并在 edge 容器内对每个可调度 Anthropic OAuth 账号跑拟真 `/v1/messages`（`probe_account_model` / `smoke_anthropic_realistic.py`）。
...
5. **其余 deployable Edge bounded-parallel rollout**：prod full smoke 绿后，`rollout-edges.sh` 对每个 edge dispatch upgrade（默认 **full**），验 log 含 `tk_edge_post_deploy_smoke: OK phase=full`。
...
# upgrade（默认 smoke_phase=full：infra + edge-native OAuth）
...
3. 取矩阵**第一个** deployable Edge 作为 canary：`dispatch-edge-deploy.sh --operation upgrade --tag=$TARGET_TAG`（**full smoke**，workflow 默认 `smoke_phase=full`），watch 到 success。
...
6. **（可选）canary main-via-edge**：`dispatch-edge-deploy.sh --operation smoke --smoke-phase main-via-edge`（验证 prod→Edge 中转）。缺 **`TK_SMOKE_API_KEY`** 则标记 partial，不 rollback。
7. **其余 deployable Edge（单命令，bounded-parallel fail-stop）**：
   ...
   脚本按 batch dispatch upgrade → watch → 验 `tk_edge_post_deploy_smoke: OK phase=full`。**默认 `--parallel 1`（顺序）**；`N>1` 仅在可接受换容器窗口影响时用。批内失败则 fail-stop。
...
| phase | 何时用 | 验证内容 |
|---|---|---|
| `infra` | 仅基础设施门禁 | 公网 runner `/v1/models`→403；SSM compose ps + localhost health |
...
| `full` | **upgrade/rollback 默认**；`operation=smoke` 默认 | infra + edge-native-oauth |

验收 checklist：

- `full`：workflow log 含 `tk_edge_post_deploy_smoke: OK phase=full`。
- `infra`：log 含 `tk_edge_post_deploy_smoke: OK phase=infra`（仅跑基础设施段时）。
