...
| rc | 摘要 verdict | 动作 |
|---|---|---|
| 0 | **green** — 所有受管平台账号 `model_mapping` 与 SSOT 一致 | 无需动作 |
| 1 | **yellow** — 有账号/Antigravity group/runtime setting 不满足 SSOT diff 或不变量 | 不 rollback 镜像；审 `post-release-account-model-mapping-check.json`，确认后走 `/tokenkey-modelops-planner` 分支 D 的 `apply-accounts --confirm yes-apply-account-model-mapping` |
