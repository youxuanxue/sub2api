...
| ⚠写侧止血：`groups.model_routing` JSON array→object（`APPLY=1`；否则 ListActiveGroups 反序列化失败 → 万能 key 全站 500） | 机械(写) | `ops/observability/remediate-model-routing-array.sh`（经 run-probe 投递；顺带 soft-delete `__tk_probe_sticky*`） |
