...
class CheckSmokeConfigTest(unittest.TestCase):
    ...
    @mock.patch.object(
        check_smoke_config,
        "_fetch_models",
        return_value=[{"id": "gemini-a"}],
    )
    def test_full_warns_on_passthrough_platforms_but_passes_when_gemini_ok(
        self, _fetch: mock.Mock
    ) -> None:
        with mock.patch("sys.argv", ["check_smoke_config.py", "--suite", "full"]), \
            mock.patch("sys.stdout", new_callable=io.StringIO), \
            mock.patch("sys.stderr", new_callable=io.StringIO) as err:
            self.assertEqual(check_smoke_config.main(), 0)
            self.assertIn("::warning::check_smoke_config[anthropic]", err.getvalue())
            self.assertIn("::warning::check_smoke_config[openai_oauth]", err.getvalue())
