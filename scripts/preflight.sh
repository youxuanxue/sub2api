...
if [ ! -x ./deploy/aws/stage0/build-cfn.sh ]; then
    ...
    if ! python3 ./ops/stage0/test_cutover_data_layer_safety.py >/dev/null 2>&1; then
        echo "  FAIL: stage0 RDS cutover approval/rollback safety gate"
        echo "        — run: python3 ops/stage0/test_cutover_data_layer_safety.py"
        errors=$((errors + 1))
    else
