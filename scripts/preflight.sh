...
# still compiles but silently injects wrong dependencies at runtime. This check
# regenerates and diffs to catch drift. Non-destructive: restores the file after.
echo ""
...
if ! command -v go >/dev/null 2>&1; then
    ...
    _wire_out=""
    _wire_rc=0
    _wire_out=$( cd backend && go generate ./cmd/server 2>&1 ) || _wire_rc=$?
    if [ "$_wire_rc" -ne 0 ]; then
        if echo "$_wire_out" | grep -q "missing go.sum entry"; then
            echo "  skip: wire tool deps missing from go.sum (run 'go get github.com/google/wire/cmd/wire@v0.7.0' to enable)"
        else
            echo "  FAIL: go generate ./cmd/server failed (exit $_wire_rc)"
            errors=$((errors + 1))
        fi
    elif ! git diff --exit-code backend/cmd/server/wire_gen.go >/dev/null 2>&1; then
        echo "  FAIL: wire_gen.go is stale — run 'go generate ./cmd/server' in backend/"
        errors=$((errors + 1))
    else
        echo "  ok: wire_gen.go is up to date"
    fi
    git checkout -- backend/cmd/server/wire_gen.go 2>/dev/null || true
fi
