...
# Strategy: compare mtimes of Wire input files against wire_gen.go. If any input
# is newer, flag as stale. This avoids the go.sum dependency issue that made the
# previous go-generate approach always skip.
echo ""
...
if [ ! -f "$_wire_gen" ]; then
    ...
    _wire_stale=0
    _wire_newest=""
    for _wf in $(find backend/ -name "wire.go" ! -name "wire_gen.go" -type f 2>/dev/null); do
        if [ "$_wf" -nt "$_wire_gen" ]; then
            _wire_stale=1
            _wire_newest="$_wf"
            break
        fi
    done
    if [ "$_wire_stale" -eq 1 ]; then
        echo "  FAIL: wire_gen.go is stale — $_wire_newest is newer. Run 'go generate ./cmd/server' in backend/"
        errors=$((errors + 1))
    else
