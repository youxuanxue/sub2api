...
# ---- sub2api: messages dispatch family registry ------------------------------
...
if ! command -v python3 >/dev/null 2>&1; then
    ...
    errors=$((errors + 1))
elif ! python3 ./scripts/checks/messages-dispatch-family-drift.py --selftest >/dev/null; then
    echo "  FAIL: messages-dispatch-family-drift selftest failed"
    errors=$((errors + 1))
elif ! python3 ./scripts/checks/messages-dispatch-family-drift.py --quiet; then
    errors=$((errors + 1))
else
    echo "  ok: messages dispatch family registry aligned"
fi
...
# Strategy: if the CURRENT BRANCH (diff vs base) modified any wire.go input,
# run `go generate ./cmd/server` and require a clean working-tree wire_gen.go.
# Runtime-only edits inside existing Provide* functions do not change the
...
if [ "$_preflight_skip_go_contracts" -eq 1 ]; then
    ...
    if [ "$_wire_inputs_changed" -eq 1 ]; then
        ...
        if [ "$_wire_rc" -ne 0 ]; then
            ...
            echo "  ok: wire_gen.go matches go generate ./cmd/server"
        fi
    else
