#!/usr/bin/env python3
"""
check-priced-serving-gate.py — verify TokenKey gateway/service hotspot hooks.

Reads `scripts/sentinels/priced-serving-gate.json` and for each entry verifies:
...
REGISTRY_PATH = REPO_ROOT / "scripts" / "sentinels" / "priced-serving-gate.json"
...
def read_file_cached(path_str: str) -> str:
    ...
    (the gemini compat hooks share one path)."""
...
def main() -> int:
    ...
    if args.json:
        ...
        if not args.quiet:
            print(
                f"priced-serving-gate sentinels: checking {len(sentinels)} entries from "
        ...
        if fail_count == 0:
            if not args.quiet:
                print(f"priced-serving-gate sentinels: PASS ({len(sentinels)}/{len(sentinels)} intact)")
        else:
            print(
                f"priced-serving-gate sentinels: FAIL ({fail_count}/{len(sentinels)} regressed)",
                file=sys.stderr,
            )
            print("  Source of truth: scripts/sentinels/priced-serving-gate.json", file=sys.stderr)
            print(
