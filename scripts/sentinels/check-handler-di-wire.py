#!/usr/bin/env python3
"""
check-handler-di-wire.py — verify TokenKey handler DI registration and Wire provider hooks.

Reads `scripts/sentinels/handler-di-wire.json` and for each entry verifies:
...
REGISTRY_PATH = REPO_ROOT / "scripts" / "sentinels" / "handler-di-wire.json"
...
def main() -> int:
    ...
    if args.json:
        ...
        if not args.quiet:
            print(
                f"handler DI/Wire sentinels: checking {len(sentinels)} entries from "
        ...
        if fail_count == 0:
            if not args.quiet:
                print(f"handler DI/Wire sentinels: PASS ({len(sentinels)}/{len(sentinels)} intact)")
        else:
            print(
                f"handler DI/Wire sentinels: FAIL ({fail_count}/{len(sentinels)} regressed)",
                file=sys.stderr,
            )
            print("  Source of truth: scripts/sentinels/handler-di-wire.json", file=sys.stderr)
            print(
