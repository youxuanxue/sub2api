#!/usr/bin/env python3
"""
check-newapi.py — verify that every load-bearing surface of TokenKey's
fifth platform `newapi` is still present in the working tree.

Reads `scripts/sentinels/newapi.json` (single source of truth) and for each
entry verifies:
...
Usage:
  ...
  python3 scripts/sentinels/check-newapi.py --quiet     # only print failures
  python3 scripts/sentinels/check-newapi.py --json      # machine-readable report
...
Why this exists:
  ...
  load-bearing surfaces in `scripts/sentinels/newapi.json` and gating both
...
REGISTRY_PATH = REPO_ROOT / "scripts" / "sentinels" / "newapi.json"
...
def main() -> int:
    ...
    if args.json:
        ...
        if fail_count == 0:
            ...
            print(
                "  Source of truth: scripts/sentinels/newapi.json",
                file=sys.stderr,
            )
            print(
