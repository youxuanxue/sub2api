#!/usr/bin/env python3
"""
check-pricing-availability.py — verify that every load-bearing tap
injection point of the pricing-availability passive observability system is
still present in the working tree.

Reads `scripts/sentinels/pricing-availability.json` (single source of truth)
and for each entry verifies:

  1. The file at `path` exists, unless `must_not_exist` is true.
  2. Every literal string in `must_contain` appears at least once in the file.
  3. No literal string in `must_not_contain` appears in the file.
...
REGISTRY_PATH = REPO_ROOT / "scripts" / "sentinels" / "pricing-availability.json"
...
def check_sentinel(entry: dict) -> tuple[bool, list[str]]:
    ...
    if entry.get("must_not_exist"):
        if file_path.exists():
            return False, [f"forbidden file exists: {path_str}"]
        return True, []
    ...
    for needle in must_not_contain:
        if needle in content:
            failures.append(f"forbidden literal `{needle}` present in {path_str}")
...
def main() -> int:
    ...
    if args.json:
        ...
        if fail_count == 0:
            ...
            print(
                "  Source of truth: scripts/sentinels/pricing-availability.json",
                file=sys.stderr,
            )
            print(
