#!/usr/bin/env python3
"""
check-antigravity.py — verify that every load-bearing surface of TokenKey's
Antigravity client fingerprint is still present in the working tree.

Reads `scripts/sentinels/antigravity.json` (single source of truth) and for each
entry verifies:
...
Usage:
  python3 scripts/sentinels/check-antigravity.py
  python3 scripts/sentinels/check-antigravity.py --quiet     # only print failures
  python3 scripts/sentinels/check-antigravity.py --json      # machine-readable report

Why this exists:
  TokenKey's Antigravity client fingerprint was silently regressed multiple times by
  upstream merges before this guard existed (see CLAUDE.md §5.x). Listing the
  load-bearing surfaces in `scripts/sentinels/antigravity.json` and gating both
...
REGISTRY_PATH = REPO_ROOT / "scripts" / "sentinels" / "antigravity.json"
...
def main() -> int:
    ...
    if args.json:
        ...
        if not args.quiet:
            print(f"antigravity sentinels: checking {len(sentinels)} entries from "
        ...
        if fail_count == 0:
            if not args.quiet:
                print(f"antigravity sentinels: PASS ({len(sentinels)}/{len(sentinels)} intact)")
        else:
            print(
                f"antigravity sentinels: FAIL ({fail_count}/{len(sentinels)} regressed)",
                file=sys.stderr,
            )
            print(
                "  Source of truth: scripts/sentinels/antigravity.json",
                file=sys.stderr,
            )
            print(
