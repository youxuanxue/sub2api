#!/usr/bin/env python3
"""
check-kiro.py — verify that every load-bearing surface of TokenKey's
sixth platform `kiro` (AWS Kiro / CodeWhisperer) is still present in the
working tree.

Reads `scripts/sentinels/kiro.json` (single source of truth) and for each
entry verifies:
...
Usage:
  ...
  python3 scripts/sentinels/check-kiro.py --quiet     # only print failures
  python3 scripts/sentinels/check-kiro.py --json      # machine-readable report

Why this exists:
  The sixth platform `kiro` rides on a vendored protocol layer
  (internal/integration/kiro, sourced from Quorinex/Kiro-Go under MIT) plus a
  thin set of TK-only injection points (PlatformKiro, KiroGatewayService,
  IsKiro, KiroTokenRefresher). An upstream merge or refactor that silently
  drops any of these makes Kiro either vanish from scheduling/UI or fail at
  runtime — exactly the failure mode that produced the newapi sentinel
  registry (CLAUDE.md §5.x). Listing the load-bearing surfaces here and gating
  both pre-commit (`scripts/preflight.sh`) and upstream merge PRs
...
REGISTRY_PATH = REPO_ROOT / "scripts" / "sentinels" / "kiro.json"
...
def main() -> int:
    ...
    if args.json:
        ...
        if not args.quiet:
            print(f"kiro sentinels: checking {len(sentinels)} entries from "
        ...
        if fail_count == 0:
            if not args.quiet:
                print(f"kiro sentinels: PASS ({len(sentinels)}/{len(sentinels)} intact)")
        else:
            print(
                f"kiro sentinels: FAIL ({fail_count}/{len(sentinels)} regressed)",
                file=sys.stderr,
            )
            print(
                "  Source of truth: scripts/sentinels/kiro.json",
                file=sys.stderr,
            )
            print(
