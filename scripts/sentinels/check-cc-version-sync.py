...
it. A cc bump only edits `cc_version` in that JSON; every other copy is
rewritten or compared by this guard. Dead snapshots and comments cannot drift
without failing check mode.
...
3. Embedded go:embed mirror — byte-compared to the source JSON, and AUTO-COPIED
   ...
   `--write` copies this mirror; a stale copy would fight sync-runtime and roll
   the fleet UA back after deploy:
