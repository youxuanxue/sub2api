...
try {
  if (mode === 'text') {
    ...
    results.push({ model, protocol: 'chat', status: 'passed', usage: response.usage });
    ...
    const first = await anthropic.messages.create({ model, max_tokens: 256, tools, messages, tool_choice: { type: 'any' } });
    ...
    results.push({ model, protocol: 'messages', status: 'passed', tools: calls.map(call => call.name), turns: [first.usage, final.usage], text });
  } else {
    for (const protocol of process.env.CURSOR_PROBE_PROTOCOL ? [process.env.CURSOR_PROBE_PROTOCOL] : ['messages', 'chat', 'responses']) {
      ...
      results.push({ model, protocol, status: 'passed', turns, text });
