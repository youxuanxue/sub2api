...
class WorkflowEdgeCoverageTest(unittest.TestCase):
    def _run(self, *, lightsail: list[str], wf_options: list[str],
             input_name: str = "edge_id", option_prefix: str = "",
             required_set: str = "lightsail-deployable", opt_out: list[dict] | None = None) -> int:
    ...
    def test_covered_passes(self) -> None:
        self.assertEqual(self._run(lightsail=["us1"], wf_options=["fra1", "us1"]), 0)
    ...
    def test_new_deployable_edge_missing_from_options_fails(self) -> None:
        ...
        self.assertEqual(self._run(lightsail=["us1", "us9"], wf_options=["us1"]), 1)

    def test_opt_out_covers_missing_edge(self) -> None:
        self.assertEqual(
            self._run(lightsail=["us1", "us9"], wf_options=["us1"],
    ...
    def test_stale_opt_out_fails(self) -> None:
        ...
        self.assertEqual(
            self._run(lightsail=["us1"], wf_options=["us1"],
                      opt_out=[{"id": "us9", "reason": "pilot only"}]),
            1,
        )
    ...
    def test_prefixed_option_pg_dump_shape(self) -> None:
        ...
        self.assertEqual(
            self._run(lightsail=["us1"], wf_options=["prod", "edge-us1", "all"],
