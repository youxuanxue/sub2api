...
2. **AC-002（cleanup hold）**：Given 生产归档操作，When plan/apply/verify，Then 只修改完整 advanced-settings 中的 cleanup bool，证明 runtime reload，并在 hold 后出现 cleanup heartbeat 时拒绝；release 恢复 receipt 记录的原 enabled 状态。
