...
1. **AC-001（确定性安全 artifact）**：Given已授权测试/probe 结果与当前 catalog projection，When publisher 构建 snapshot，Then输出固定 schema/hash，只含 client/version/protocol/transport/model/verdict/limitation/time/runtime/evidence kind，secret/internal-topology gate 为零。
...
3. **AC-003（freshness/runtime）**：Given row 超过 policy freshness 或 runtime/client anchor 不匹配，When public API 读取，Then verified 降级为 compatible/unknown，并保留诚实的 last verified time，不继续绿色展示。
...
5. **AC-005（catalog membership）**：Given snapshot model 不在 US-044 当前可售 projection，When验证 artifact，Then该 row不能成为 public positive candidate；未知/历史信息不得抬高当前可售覆盖。
6. **AC-006（公开限制码）**：Given limited row，When用户展开，Then只看到批准的用户影响码、解释与 workaround，不看到 pool、entitlement、routing、account 或 Edge 原因。
7. **AC-007（API fail closed）**：Given snapshot schema/hash/runtime/catalog validation 失败，When请求 `/api/v1/public/compatibility`，Then返回 503 `compatibility_evidence_unavailable`，不把上一份 artifact 冒充当前。
