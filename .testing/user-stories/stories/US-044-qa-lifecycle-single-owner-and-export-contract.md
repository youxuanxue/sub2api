...
2. prod trajectory/self-export worker、gateway download proxy 与 `qa_exports_tmp` orphan cleaner 不存在。
3. lifecycle sentinel 拒绝 prod fallback、第二 deletion owner、固定时龄 DROP 与 export-orphan runtime。
4. rollout 将 repository readiness 与 `single_owner_not_activated` observed state 分开记录。
