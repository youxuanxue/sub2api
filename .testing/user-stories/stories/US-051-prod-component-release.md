...
- Priority: P1
- As a / I want / So that: As a gateway operator, I want separate gateway and QA deployment entries with verified component baselines, so that gateway releases do not wait for unrelated QA work.
- Trace: docs/approved/prod-component-release.md; docs/approved/design-split-deploy-qa-bundle.md; user approval 2026-09-14
- Risk Focus:
...
3. AC-003: QA deploy requires compatible target and observed gateway contracts; health and canary must pass before recording their combination.
4. AC-004: Routine gateway releases do not repeat the publisher canary; explicit QA deploy/canary-only runs do.
