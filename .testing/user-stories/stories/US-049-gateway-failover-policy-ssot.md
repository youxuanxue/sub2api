...
1. AC-001 (正向): Given 任一已支持 profile 的账号级或瞬时失败 When adapter 提交 observation Then 全局 policy 返回 `RetryNextAccount=true`。
2. AC-002 (负向): Given terminal request semantic 或未知 profile/semantic When 全局 policy 判定 Then fail closed，不切换账号。
...
5. AC-005 (机械守卫): Given 上游 merge 删除 owner、测试或 adapter 接线，新增未委托全局 owner 的 `shouldFailover` facade，或恢复 `ShouldFailover bool` 字段 When 运行 gateway sentinel Then preflight 失败。

## Assertions

- semantic 先于 status，terminal 503 不 failover，retryable 400 会 failover。
...
- `TestUS049_`: PASS.
- Full `internal/service` unit package: PASS (`ok github.com/Wei-Shaw/sub2api/internal/service 113.267s`).
- Gateway TK sentinel registry and global invariant: PASS (`853/853 intact`).
