...
5. AC-005 (机械守卫): Given 上游 merge 删除 owner、测试或 adapter 接线，新增未委托全局 owner 的 `shouldFailover` / `retryNextAccount` facade，绕过 `ShouldRetryNextAccount` runtime choke point，恢复 decision bool，在 adapter 直接写 retry/stop action，或用 verdict 型 semantic 伪装决策 When 运行 gateway sentinel Then preflight 失败。
...
- 所有 handler 对 `UpstreamFailoverError` 的最终 retry/stop 判定都经过全局 owner；legacy 零值仍 retry，未知 action fail closed。
...
- `TestUS049_`: PASS.
- Full `internal/service` unit package: PASS (`ok github.com/Wei-Shaw/sub2api/internal/service 113.414s`).
