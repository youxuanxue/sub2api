...
- 验证手动跑：`gh workflow run prod-ops.yml -f operation=diagnostics -f target_selector=prod -f since_hours=24 -R youxuanxue/sub2api`
...
`prod-ops.yml`，再下载 artifact。AWS OIDC → SSM → EC2 链路完全不动。
