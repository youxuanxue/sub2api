...
- `t4g.large` 已在模板 `AllowedValues`；**必须先 merge 本 PR 的 UserData/SSM 修复**（否则 change-set 会因 16 KiB 超限失败），再执行下方 deploy。
...
aws cloudformation deploy --region "$REGION" --stack-name "$STACK" \
  ...
  --parameter-overrides InstanceType="$TARGET"
...
# 3) 验证：新实例规格 → SSM 重新上线 → /health → 业务冒烟
...
# 关键容器 healthy + 业务冒烟：参照 §升级 SOP 的 smoke（ops/stage0/post_deploy_smoke.sh）。

# 回退：把 TARGET 改回 t4g.small 重跑步骤 2（同样 1–3 min replace，数据同样保留）。
```
