...
- `POST_DEPLOY_SMOKE_OPENAI_OAUTH_API_KEY` —— 绑到 OpenAI OAuth/codex 平台分组
  的 api_key，覆盖 `/v1/chat/completions` 上 `usage.completion_tokens_details.reasoning_tokens` 透传不变量（chatgpt.com codex 后端会返回 reasoning_tokens 计数即使不返回 reasoning 文本）。
