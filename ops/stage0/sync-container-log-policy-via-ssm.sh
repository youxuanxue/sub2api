...
jq -n --arg compose "$COMPOSE_B64" '{
  commands: [
    ...
    "MAX_SIZE=$(sudo docker inspect tokenkey-caddy --format '\''{{index .HostConfig.LogConfig.Config \"max-size\"}}'\''); [ -n \"$MAX_SIZE\" ]",
    "MAX_FILE=$(sudo docker inspect tokenkey-caddy --format '\''{{index .HostConfig.LogConfig.Config \"max-file\"}}'\''); [ \"$MAX_FILE\" -ge 2 ]",
