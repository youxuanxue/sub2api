...
case "${TIMER_STATE}" in
  disabled)
    ...
    ;;
  enabled|auto)
    owner_command="sudo bash -c 'set -euo pipefail; active=\$(docker exec tokenkey-postgres psql -U tokenkey -d tokenkey -X -A -t -v ON_ERROR_STOP=1 -c \"SELECT count(*) FROM qa_lifecycle_receipts WHERE phase=concat(chr(115),chr(105),chr(110),chr(103),chr(108),chr(101),chr(95),chr(111),chr(119),chr(110),chr(101),chr(114),chr(95),chr(97),chr(99),chr(116),chr(105),chr(118),chr(97),chr(116),chr(101))\" | tr -d \"[:space:]\"); case \"\${active}\" in 0) systemctl enable --now tokenkey-qa-boundary.timer; test \"\$(systemctl is-enabled tokenkey-qa-boundary.timer)\" = enabled; test \"\$(systemctl is-active tokenkey-qa-boundary.timer)\" = active ;; 1) systemctl disable --now tokenkey-qa-boundary.timer; test \"\$(systemctl is-enabled tokenkey-qa-boundary.timer)\" = disabled; test \"\$(systemctl is-active tokenkey-qa-boundary.timer)\" = inactive ;; *) echo \"invalid single-owner activation receipt count: \${active}\" >&2; exit 1 ;; esac'"
...
jq -n \
  ...
  {commands:[
    "set -euo pipefail",
    "qa_single_owner_active=$(docker exec tokenkey-postgres psql -U tokenkey -d tokenkey -X -A -t -v ON_ERROR_STOP=1 -c \"SELECT count(*) FROM qa_lifecycle_receipts WHERE phase=concat(chr(115),chr(105),chr(110),chr(103),chr(108),chr(101),chr(95),chr(111),chr(119),chr(110),chr(101),chr(114),chr(95),chr(97),chr(99),chr(116),chr(105),chr(118),chr(97),chr(116),chr(101))\" | tr -d \"[:space:]\"); case \"${qa_single_owner_active}\" in 0|1) ;; *) echo \"invalid single-owner activation receipt count: ${qa_single_owner_active}\" >&2; exit 1 ;; esac; if sudo systemctl is-enabled --quiet tokenkey-qa-boundary.timer; then qa_boundary_enabled=1; else qa_boundary_enabled=0; fi; if sudo systemctl is-active --quiet tokenkey-qa-boundary.timer; then qa_boundary_active=1; else qa_boundary_active=0; fi; qa_sync_committed=0; qa_sync_restore() { qa_sync_rc=$?; trap - EXIT; if [ \"${qa_sync_committed}\" != 1 ]; then if [ \"${qa_single_owner_active}\" = 1 ]; then sudo systemctl disable tokenkey-qa-boundary.timer >/dev/null 2>&1 || true; sudo systemctl stop tokenkey-qa-boundary.timer >/dev/null 2>&1 || true; else if [ \"${qa_boundary_enabled}\" = 1 ]; then sudo systemctl enable tokenkey-qa-boundary.timer >/dev/null 2>&1 || true; else sudo systemctl disable tokenkey-qa-boundary.timer >/dev/null 2>&1 || true; fi; if [ \"${qa_boundary_active}\" = 1 ]; then sudo systemctl start tokenkey-qa-boundary.timer >/dev/null 2>&1 || true; else sudo systemctl stop tokenkey-qa-boundary.timer >/dev/null 2>&1 || true; fi; fi; fi; exit \"${qa_sync_rc}\"; }; trap qa_sync_restore EXIT",
