...
commands = [
    ...
    "UPDATE users SET password_hash = crypt('${NEW_PASSWORD}', gen_salt('bf', 10)), updated_at = NOW() WHERE email = '${ADMIN_EMAIL}' AND role = 'admin';",
    "EOSQL",
    "UPDATED_COUNT=$(sudo docker exec tokenkey-postgres psql -U tokenkey -d tokenkey -v ON_ERROR_STOP=1 -tA -c \"SELECT COUNT(1) FROM users WHERE email = '${ADMIN_EMAIL}' AND role = 'admin';\")",
