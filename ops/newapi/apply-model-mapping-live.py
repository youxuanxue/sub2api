#!/usr/bin/env python3
"""Emergency removal of model_mapping keys from one prod newapi account.

This is deliberately removal-only. New model floors go through the reviewed model-surface
bundle plus ``ops/pricing/modelops.py activate``; this tool cannot add or rewrite a
mapping. Removal remains as an incident stop-loss path when an existing mapping routes
traffic to an account that cannot serve it. Correct the companion migration or compiled
owner in the same change so a later release cannot restore the bad key.
...
-----------
  ...
  --selftest  Offline unit test of validation/removal SQL building (no AWS).
...
the shared stdin (silent truncation, still rc=0). PROD ONLY (newapi accounts live only
on the prod control-plane DB; edges are anthropic relays).
"""
...
def build_removals(remove_keys: list[str]) -> list[str]:
    ...
    A key this tool writes into the `- text[]` literal can never carry a quote or space
    that breaks the SQL literal.
    """
    out: set[str] = set()
...
def build_remove_sql(account_id: int, name: str, platform: str, channel_type: int,
    ...
    Uses jsonb `- text[]`; this incident tool has no merge/addition primitive. Idempotent:
    `-` on an absent key is a no-op, and every other mapping entry is left untouched.

    A SINGLE data-modifying-CTE statement is atomic on its own.
    """
    return (
        "WITH upd AS (\n"
        "  UPDATE accounts\n"
...
def cmd_remove_live(args) -> int:
    """Hot-remove model_mapping keys from one newapi account.
...
def _selftest() -> int:
    ...
    if failures:
        ...
        for f in failures:
            print(f"  - {f}")
        return 1
    print("selftest ok: removals / validation / keys-array / SQL shape")
    return 0
...
def main() -> int:
    ...
    r = sub.add_parser("remove-live", help="hot-remove model_mapping keys from prod")
