...
# 避免引号断开或改写 \set 元命令。
_ACCOUNT_NAME_SAFE_RE = re.compile(r"^[a-zA-Z0-9._-]+$")
...
def validate_account_name_for_psql(account_name: str) -> None:
    if not account_name or len(account_name) > 256:
        fail(f"invalid account_name length for SQL apply: {account_name!r}", code=2)
    if _ACCOUNT_NAME_SAFE_RE.fullmatch(account_name) is None:
        fail(
            "account_name must match "
            f"{_ACCOUNT_NAME_SAFE_RE.pattern!r} (got {account_name!r}); "
            "refuse unsafe psql literal injection.",
            code=2,
        )
...
def cmd_plan(args: argparse.Namespace) -> int:
    ...
    # stale_count 包含「priority 已为最优因而无 apply action」的 stale 账号；
    # any_stale 不可仅从 actions 推导。
    any_stale_buckets = any((s.get("stale_count") or 0) > 0 for s in all_summaries)
    any_stale_actions = any(a["ranking"]["stale"] for a in all_actions)
    any_stale = any_stale_buckets or any_stale_actions
...
def render_apply_sql(account_name: str, new_priority: int) -> str:
    validate_account_name_for_psql(account_name)
    body = _read_template()
    header = (
