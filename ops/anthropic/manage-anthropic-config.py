...
def render_runtime_sync_shell(ua_version: str) -> str:
    """Remote shell: upsert settings.claude_code_user_agent_version + DEL fingerprint:*."""
    quoted = ua_version.replace("'", "''")
    key = RUNTIME_SYNC_SETTING_KEY
    return (
        "# Generated by manage-anthropic-config.py sync-runtime\n"
        "set -u\n"
        "PSQL='sudo docker exec tokenkey-postgres psql -U tokenkey -d tokenkey -X -A -t -v ON_ERROR_STOP=1'\n"
        "RC='env -u REDISCLI_AUTH sudo docker exec tokenkey-redis redis-cli'\n"
        "echo '=== settings_upsert ==='\n"
        f"$PSQL -c \"INSERT INTO settings (key, value, updated_at) VALUES "
        f"('{key}', '{quoted}', NOW()) "
...
def _run_runtime_sync(targets: list[str], ua_version: str, job_dir: pathlib.Path | None
                      ) -> tuple[bool, list[dict]]:
    results: list[dict] = []
    ok_all = True
    for target in targets:
        label, region, instance_id = _resolve_runtime_sync_target(target)
        shell = render_runtime_sync_shell(ua_version)
...
def cmd_sync_runtime(args: argparse.Namespace) -> int:
    ...
    ok, results = _run_runtime_sync(targets, ua_version, job_dir)
    report = {
        "version": 1,
    ...
    if args.json:
        ...
        print(f"sync-runtime: success={ok} targets={len(targets)} ua={ua_version}")
        for r in results:
...
def main() -> int:
    ...
    sp.add_argument(
        ...
        action="store_true",
        help=(
            "after a successful apply, upsert claude_code_user_agent_version in settings "
            "and DEL Redis fingerprint:{oauth_account_id} on affected edge targets "
    ...
    sp = sub.add_parser(
        "sync-runtime",
        help="upsert claude_code_user_agent_version + flush Redis OAuth fingerprint cache",
    )
    sp.add_argument(
        "--target",
        required=True,
