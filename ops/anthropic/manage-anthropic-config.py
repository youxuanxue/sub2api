...
def render_runtime_sync_shell(ua_version: str, mimicry_manifest_json: str | None = None) -> str:
    ...
    quoted_ua = _sql_quote_literal(ua_version)
    ...
    quoted_manifest = _sql_quote_literal(manifest_json)
    ...
    return (
        ...
        "echo '=== settings_upsert_ua ==='\n"
        f"$PSQL -c \"INSERT INTO settings (key, value, updated_at) VALUES "
        f"('{ua_key}', '{quoted_ua}', NOW()) "
        "ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = NOW() "
        "RETURNING key, value;\"\n"
        "echo '=== settings_upsert_http_mimicry ==='\n"
        f"$PSQL -c \"INSERT INTO settings (key, value, updated_at) VALUES "
        f"('{mimicry_key}', '{quoted_manifest}', NOW()) "
        "ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value, updated_at = NOW() "
        "RETURNING key;\"\n"
