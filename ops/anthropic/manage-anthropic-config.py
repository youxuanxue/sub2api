...
def render_redis_cache_drift_shell() -> str:
    """Remote read-only shell: dump the two Redis config blobs (+ TTL) and the
    two authoritative DB tables, delimited by @@RDRIFT: markers for the parser.
    ...
    return (
        ...
        "RC='env -u REDISCLI_AUTH sudo docker exec tokenkey-redis redis-cli'\n"
        f"echo '{m}tls_redis'\n"
        f"$RC GET {REDIS_DRIFT_TLS_CACHE_KEY} 2>/dev/null || true\n"
        f"echo '{m}tls_ttl'\n"
        f"$RC TTL {REDIS_DRIFT_TLS_CACHE_KEY} 2>/dev/null || true\n"
        f"echo '{m}tiers_redis'\n"
        f"$RC GET {REDIS_DRIFT_TIERS_CACHE_KEY} 2>/dev/null || true\n"
        f"echo '{m}tiers_ttl'\n"
        f"$RC TTL {REDIS_DRIFT_TIERS_CACHE_KEY} 2>/dev/null || true\n"
        f"echo '{m}tls_db'\n"
