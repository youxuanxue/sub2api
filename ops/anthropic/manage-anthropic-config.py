...
def render_runtime_sync_shell(ua_version: str, mimicry_manifest_json: str | None = None) -> str:
    ...
    # redis_rc tracks any Redis invalidation failure across all redis-cli calls
    # so the script can report success/failure for the invalidation phase
    # independently from the settings upsert (which already succeeded by that
    # point). `|| true` is gone from tls_fingerprint_profiles DEL/PUBLISH so a
    # genuine Redis outage does NOT hide behind "ok": true.
    #
    # `env -u REDISCLI_AUTH` must run INSIDE the container (after `docker exec`)
    # because docker exec inherits only the container's env, not the host's.
    # The original placement (`env -u ... sudo docker exec ...`) unset the
    # host-side variable, which has no effect — the container's REDISCLI_AUTH
    # (set to `${REDIS_PASSWORD:-}` by docker-compose) persisted and caused
    # spurious AUTH on hosts where REDIS_PASSWORD was empty.
    return (
        ...
        "RC='sudo docker exec tokenkey-redis env -u REDISCLI_AUTH redis-cli'\n"
        "redis_rc=0\n"
        ...
        "  for id in $ids; do fk=\"fingerprint:${id}\"; out=\"$($RC DEL \"$fk\" 2>&1)\" || "
        "{ redis_rc=$?; out=\"FAIL(rc=$redis_rc): $out\"; }; "
        "echo \"DEL ${fk} => ${out}\"; done\n"
        "fi\n"
        "echo '=== redis_tls_fingerprint_profiles_invalidate ==='\n"
        "out=\"$($RC DEL tls_fingerprint_profiles 2>&1)\" || "
        "{ redis_rc=$?; out=\"FAIL(rc=$redis_rc): $out\"; }\n"
        "echo \"DEL tls_fingerprint_profiles => ${out}\"\n"
        "out=\"$($RC PUBLISH tls_fingerprint_profiles_updated sync-runtime 2>&1)\" || "
        "{ redis_rc=$?; out=\"FAIL(rc=$redis_rc): $out\"; }\n"
        ...
        "  echo 'REDIS_INVALIDATION_FAILED' >&2\n"
        "  exit 1\n"
        "fi\n"
    )
