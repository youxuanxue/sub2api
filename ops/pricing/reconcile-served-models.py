#!/usr/bin/env python3
"""Read-only served-model reconcile planner for TokenKey.

This is intentionally NOT a background writer. It automates the deterministic
parts of served-model operations:
  ...
  * produce probe / pricing / mapping-review plans.
...
def print_plan(plan: dict[str, Any]) -> None:
    ...
    print("served-model reconcile plan")
    print(
    ...
    print_section(
        "live mapping missing manifest entries",
...
def _selftest() -> int:
    ...
    if failures:
        ...
        for failure in failures:
            print(f"  - {failure}")
        return 1
    print("selftest ok: reconcile planner parsing/probe/mapping helpers behave")
    return 0


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="TokenKey served-model reconcile planner")
    parser.add_argument("--selftest", action="store_true", help="run offline selftest")
    sub = parser.add_subparsers(dest="command")

    plan = sub.add_parser("plan", help="build a read-only reconcile plan")
