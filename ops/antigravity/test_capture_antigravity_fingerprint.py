...
def _synth_baseline() -> dict:
    return {
        "ua_version": "1.23.2",
        "ua_format": "antigravity/%s windows/amd64",
        "client_id": "client-id",
        ...
        "plugin_type": "GEMINI",
        "x_goog_api_client": "gl-node/22.21.1",
    }


def _full_http(ua_version="1.23.2", os_arch="windows/amd64") -> dict:
    return {
        "user_agent": f"antigravity/{ua_version} {os_arch}",
        "body_user_agent": "antigravity",
        "ide_type": "ANTIGRAVITY",
        "ide_name": "antigravity",
        "platform": "PLATFORM_UNSPECIFIED",
...
class BaselineLoadTests(unittest.TestCase):
    def test_live_constants_load(self):
        ...
        self.assertTrue(b["x_goog_api_client"].startswith("gl-node/"))
    ...
    def test_expected_user_agent_renders_windows_amd64(self):
        b = _synth_baseline()
        self.assertEqual(eng.expected_user_agent(b), "antigravity/1.23.2 windows/amd64")
...
class ParseUaTests(unittest.TestCase):
    def test_parses_version_and_os_arch(self):
        ...
        self.assertEqual(eng.parse_ua("antigravity/1.104.0 darwin/arm64"), ("1.104.0", "darwin/arm64"))
...
class DiffTests(unittest.TestCase):
    ...
    def test_gl_node_version_drift_is_actionable(self):
        http = _full_http()
...
class BundleRoundtripTests(unittest.TestCase):
    def test_bundle_from_artifacts_roundtrip(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(bundle["http"]["user_agent"], "antigravity/1.23.2 windows/amd64")
