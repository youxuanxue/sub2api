...
# beside the probe; the repo path covers local runs. Importing beats re-deriving:
...
tk_resolve = _tk.resolve
_ACTIVE_COLOR_PATH = _tk.ACTIVE_COLOR_PATH

def resolve_container(container: str) -> tuple[str | None, list[str]]:
    # Canonical resolver (ops/lib/resolve_app_container.py) owns the rules;
    # run-probe.sh uploads it next to this probe. None means unresolved, and
    # unresolved must stay unresolved rather than becoming a positional guess.
    return tk_resolve(container, active_color_file=_ACTIVE_COLOR_PATH)
