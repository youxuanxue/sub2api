#!/bin/bash
# probe-data-layer-capacity-prototype.sh — dormant bounded capacity snapshot prototype.
#
# This file is not wired to any workflow or scheduled prod consumer. A later
# activation requires separate approval. Pure read-only: bounded psql
# SELECT + host `df`. Emits catalog size/cardinality, recent row growth, and
# data-volume free space as tagged JSON lines for data_layer_capacity_verdict_prototype.py:
#
...
# so the threshold logic stays unit-testable (data_layer_capacity_verdict_prototype.py --selftest).
set -u
