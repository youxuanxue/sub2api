...
PY="$SCRIPT_DIR/capture_gemini_fingerprint.py"
usage() {
  cat <<'EOF'
Usage:
  capture-gemini-fingerprint.sh check env
  capture-gemini-fingerprint.sh show-baseline
  capture-gemini-fingerprint.sh diff
  capture-gemini-fingerprint.sh check
  capture-gemini-fingerprint.sh capture [--out-dir DIR]
EOF
}
