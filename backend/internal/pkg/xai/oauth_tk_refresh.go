// oauth_tk_refresh.go — TokenKey refresh-only xAI/Grok OAuth helpers.
//
// TokenKey's grok seventh platform onboards an account by storing a
// refresh_token the operator mints out-of-band with the xAI Grok CLI, then
// refreshes it server-side from here (mirroring how the Kiro refresher calls
// the vendored kiroproto.RefreshToken). This file is intentionally
// dependency-free (only stdlib).
//
// It is kept as a TK companion alongside the upstream-owned oauth.go (which
// provides the interactive authorization-code + PKCE / session-store / quota
// machinery) so both grok onboarding paths coexist without divergence churn in
// the upstream file. Shared identifiers that upstream already defines (e.g.
// DiscoveryURL) are NOT redeclared here.
//
