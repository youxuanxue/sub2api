...
func nextCLIPatchVersion(t *testing.T) string {
	t.Helper()
	var major, minor, patch int
	if n, err := fmt.Sscanf(CLICurrentVersion, "%d.%d.%d", &major, &minor, &patch); err != nil || n != 3 {
		t.Fatalf("invalid CLI baseline %q", CLICurrentVersion)
	}
...
func TestIsSupportedCLIVersion(t *testing.T) {
	cases := []struct {
		...
		{"高于基线的补丁位", nextCLIPatchVersion(t), true},
		{"高于基线的次版本", "2.2.0", true},
		{"高于基线的主版本", "3.0.0", true},
		{"低于基线", "2.1.219", false},
		{"远低于基线", "1.0.0", false},
