...
func TestRehomeDefaultMonthlyCopyOnlyLeavesDefaultVisibleUntilFinalize(t *testing.T) {
	...
	mock.ExpectExec("CREATE TABLE \"qa_records_2026_08\" PARTITION OF \"qa_records\"").
		WillReturnResult(sqlmock.NewResult(0, 0))
	mock.ExpectQuery("SELECT COUNT\\(\\*\\) FROM \"qa_records_2026_08\"").
		WithArgs(start, end).
		WillReturnRows(sqlmock.NewRows([]string{"count"}).AddRow(int64(0)))
...
func TestRehomeDefaultMonthlyBudgetExhaustedKeepsDefaultRowsVisible(t *testing.T) {
	db, mock, err := sqlmock.New(sqlmock.QueryMatcherOption(sqlmock.QueryMatcherRegexp))
	if err != nil {
		t.Fatalf("sqlmock.New: %v", err)
	}
