...
func TestCalculateCost_OpenAIGPT54LongContextLeavesUnpricedCacheCreationAtZero(t *testing.T) {
	svc := newTestBillingService()
	...
	require.Zero(t, cost.CacheCreationCost,
		"long-context policy must not invent a cache-write price absent from the registry")
}

func TestCalculateCost_OpenAIGPT54NoLongContextLeavesUnpricedCacheCreationAtZero(t *testing.T) {
	svc := newTestBillingService()
	...
	require.Zero(t, cost.CacheCreationCost,
		"cache-write usage must stay unpriced when the registry has no cache-write dimension")
}
...
func TestGetFallbackPricing_FamilyMatching(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "gpt realtime unknown → excluded from chat floor (nil)", model: "gpt-realtime-x-unknown", expectNilPricing: true},
		{
			...
			expectedInput:     tkCNYPerMTokToUSDPerToken(4.5),
			expectedOutput:    floatPtr(tkCNYPerMTokToUSDPerToken(13.5)),
			expectedCacheRead: floatPtr(tkCNYPerMTokToUSDPerToken(0.15)),
		},
		{
			name:              "deepseek flash",
			model:             "deepseek-flash",
			expectedInput:     tkCNYPerMTokToUSDPerToken(1),
			expectedOutput:    floatPtr(tkCNYPerMTokToUSDPerToken(4)),
			expectedCacheRead: floatPtr(tkCNYPerMTokToUSDPerToken(0.02)),
		},
		{
		...
		{
			// Legacy compatibility names follow the stable Flash pricing policy.
			name:              "deepseek chat compatibility maps to flash",
			model:             "deepseek-chat",
			expectedInput:     tkCNYPerMTokToUSDPerToken(1),
			expectedOutput:    floatPtr(tkCNYPerMTokToUSDPerToken(4)),
			expectedCacheRead: floatPtr(tkCNYPerMTokToUSDPerToken(0.02)),
		...
		{
			name:              "deepseek reasoner compatibility maps to flash",
		...
		},
		{
			...
			expectedOutput:    floatPtr(glmCNYPerMTokPreTax(2.8)),
			expectedCacheRead: floatPtr(glmCNYPerMTokPreTax(0.23)),
		},
		{
		...
		},
		{
			...
			expectedCacheRead: floatPtr(2.8e-7),
		},
		{
			name:              "glm 5 base",
			model:             "glm-5",
...
func TestCalculateCost_DoubaoEmbeddingVisionDifferentialInput(t *testing.T) {
	...
	wantTextInput := float64(1312) * textRate
	wantImageInput := float64(28) * imageRate
	...
	require.InDelta(t, float64(10)*imageRate, costWeird.ImageInputCost, 1e-15)
	require.InDelta(t, float64(10)*imageRate, costWeird.TotalCost, 1e-15)
}
...
func TestComputeTokenBreakdown_GptImage2ImageEditIssue4386(t *testing.T) {
	...
	cost := svc.computeTokenBreakdown(pricing, tokens, 1.0, "", false, true)
...
func TestGetModelPricing_Grok46OfficialFallback(t *testing.T) {
	...
	for _, model := range []string{"grok", "grok-4.6", "grok-4.6-latest", xai.DefaultTextModel} {
		model := model
		t.Run(model, func(t *testing.T) {
			pricing, err := svc.GetModelPricing(model)
			require.NoError(t, err)
...
func TestCalculateCost_Grok46LongContextAppliesOfficialTier(t *testing.T) {
	svc := newTestBillingService()
	...
	expectedOutput := float64(tokens.OutputTokens) * 6e-6 * 2.0
	require.InDelta(t, expectedInput, cost.InputCost, 1e-10)
	require.InDelta(t, expectedCacheRead, cost.CacheReadCost, 1e-10)
	require.InDelta(t, expectedOutput, cost.OutputCost, 1e-10)
	require.True(t, cost.LongContextBillingApplied)
...
func TestCalculateCostUnified_GroupLongContextToggleUsesPresetLadder(t *testing.T) {
	...
	disabled, err := svc.CalculateCostUnified(CostInput{
		Model: "grok-4.6", Group: off, Tokens: tokens, RateMultiplier: 1, Resolver: resolver,
	})
	require.NoError(t, err)

	on := &Group{LongContextPricingEnabled: true}
	enabled, err := svc.CalculateCostUnified(CostInput{
		Model: "grok-4.6", Group: on, Tokens: tokens, RateMultiplier: 1, Resolver: resolver,
	})
	require.NoError(t, err)
...
func TestGetModelPricing_UnknownGrokTextFallsBackToDefaultTextCard(t *testing.T) {
	svc := newTestBillingService()
	...
	for _, model := range []string{
		...
		require.Error(t, err, "non-text grok family %s must not inherit default token rates", model)
		require.ErrorIs(t, err, ErrModelPricingUnavailable)
	}

	// Known cards stay on their own rate, not the default family floor.
...
func TestNewModelPricingChannelOverridesAndFamilyIsolation(t *testing.T) {
	...
	for _, model := range []string{"gpt-6-sol", "gpt-6-luna", "claude-opus-5-5"} {
		t.Run(model, func(t *testing.T) {
			...
			require.NoError(t, err)
			cost := svc.computeTokenBreakdown(prices, UsageTokens{InputTokens: 300000, CacheReadTokens: 1000, CacheCreationTokens: 1000, OutputTokens: 1000}, 1, "priority", true, false)
