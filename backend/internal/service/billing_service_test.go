...
func TestCalculateCost_DoubaoEmbeddingVisionDifferentialInput(t *testing.T) {
	...
	require.InDelta(t, float64(1312)*textRate, cost.InputCost, 1e-15, "InputCost 仅计文本输入")
	require.InDelta(t, float64(28)*imageRate, cost.ImageInputCost, 1e-15, "ImageInputCost 单独计图片输入")
	require.InDelta(t, wantMixed, cost.TotalCost, 1e-15, "TotalCost 口径不变")
