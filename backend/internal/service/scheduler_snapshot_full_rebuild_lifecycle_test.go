...
func TestSchedulerFullRebuildActiveTombstoneDoesNotBlockFollowingGroupEvent(t *testing.T) {
	...
	require.Equal(t, 3*schedulerCanonicalAccountLoadCount(groupID), accounts.callCount())
}
...
func TestSchedulerFullRebuildFreshActivePreparesEveryTokenBeforeFirstDB(t *testing.T) {
	...
	require.Equal(t, 2*schedulerCanonicalAccountLoadCount(groupID)+1, accounts.callCount())
	require.Equal(t, schedulerCanonicalAccountLoadCount(groupID)+1, accounts.groupCallCount(groupID))
...
func TestSchedulerFullRebuildPreservesGroupZeroActiveHistoricalAndInvalidRegistryBuckets(t *testing.T) {
	...
	require.Equal(t, 2*schedulerCanonicalAccountLoadCount(groupID)+3, accounts.callCount())
	groups.mu.Lock()
	require.Equal(t, 1, groups.listCalls)
	groups.mu.Unlock()
...
func TestSchedulerFullRebuildActiveTombstoneFreshInactiveOrMissingFiltersAllGroupTasks(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			require.Equal(t, schedulerCanonicalAccountLoadCount(0), accounts.groupCallCount(0))
			require.Empty(t, cache.tokens())
...
func TestSchedulerFullRebuildPartialLifecycleFailureReturnsBeforeDBAndRetries(t *testing.T) {
	...
	require.Equal(t, schedulerCanonicalAccountLoadCount(0), accounts.callCount())
	require.Empty(t, cache.tokens())
}
...
func TestSchedulerFullRebuildSimpleModePreservesRegistryWithoutLifecycleAuthority(t *testing.T) {
	...
	require.Equal(t, schedulerCanonicalAccountLoadCount(0)+len(registered), accounts.callCount())
	require.Equal(t, schedulerCanonicalAccountLoadCount(0)+len(registered), accounts.groupCallCount(0))
	require.Empty(t, cache.retiredBuckets())
	require.Empty(t, cache.tokens())
...
func TestSchedulerFullRebuildFreshReopenLockBusyRetriesWithoutBlockingOrdinaryTasks(t *testing.T) {
	...
	require.Equal(t, 2*schedulerCanonicalAccountLoadCount(groupID), accounts.callCount())

	svc.pollOutbox()
	require.Equal(t, int64(1), cache.currentWatermark())
	require.Equal(t, 4*schedulerCanonicalAccountLoadCount(groupID), accounts.callCount())
