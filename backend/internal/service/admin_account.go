...
func (s *adminServiceImpl) ListAccounts(ctx context.Context, page, pageSize int, platform, accountType, status, search string, groupID int64, privacyMode string, sortBy, sortOrder string, channelType int) ([]Account, int64, error) {
	...
	accounts, result, err := s.accountRepo.ListWithFilters(ctx, params, platform, accountType, status, search, groupID, privacyMode, channelType)
	if err != nil {
		return nil, 0, err
	}
...
func (s *adminServiceImpl) ListAccountsForSchedulerScoreFilter(ctx context.Context, platform, accountType, status, search string, groupID int64, privacyMode string) ([]Account, error) {
	...
	return s.accountRepo.ListAllWithFilters(ctx, platform, accountType, status, search, groupID, privacyMode, 0)
}
...
func (s *adminServiceImpl) buildDuplicateAccount(ctx context.Context, source *Account, operationID string) (*Account, []AccountGroup, error) {
	...
	// Retain the Worker declaration for initialization, never its session or lease.
	if _, declared := credentials["gemini_web"].(map[string]any); declared {
		credentials["gemini_web"] = map[string]any{}
	} else {
...
func (s *adminServiceImpl) BulkUpdateAccounts(ctx context.Context, input *BulkUpdateAccountsInput) (*BulkUpdateAccountsResult, error) {
	...
	delete(input.Extra, SupportedProtocolsExtraKey)
...
func (s *adminServiceImpl) ClearAccountError(ctx context.Context, id int64) (*Account, error) {
	if s.rateLimitService != nil {
		...
		if err := s.rateLimitService.RecoverSupplierCredentialPeers(ctx, account); err != nil {
			return nil, err
		}
	}
