	...
	if reasoningKnown {
		...
		if reasoningValue {
			...
			if len(levels) == 0 {
				...
				if !codexStringSliceContains(levels, sharedDefault) {
...
func applyUpstreamModelMetadataToCodexDescriptor(
	...
	if metadata.reasoningConflict {
		...
		if len(levels) == 0 {
			...
			if !codexStringSliceContains(levels, defaultLevel) {
...
func codexStringSliceContains(values []string, target string) bool {
	if target == "" {
		return false
	}
