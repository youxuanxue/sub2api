...
func (s *AccountTestService) fetchOpenAICodexOAuthUpstreamModels(ctx context.Context, account *Account) ([]string, []byte, error) {
	if s == nil || s.openAIGatewayService == nil {
		return nil, nil, newUpstreamModelSyncConfigError("OpenAI gateway service is not configured for Codex model sync", nil)
	}
	manifest, err := s.openAIGatewayService.FetchCodexModelsManifest(ctx, account, "", "")
	if err != nil {
		return nil, nil, newUpstreamModelSyncUpstreamError("Failed to fetch Codex models manifest", err)
	}
	...
	if err != nil {
		return nil, nil, newUpstreamModelSyncUpstreamError("Codex models manifest response was not valid JSON", err)
	}
	if len(models) == 0 {
		return nil, nil, newUpstreamModelSyncUpstreamError("Upstream returned no supported models", nil)
	}
