...
func TestParseClaudeCodeHTTPMimicryManifest_Valid(t *testing.T) {
	raw := `{"schema_version":1,"cc_version":"2.1.153","sonnet_opus":["oauth-2025-04-20","claude-code-20250219"],"haiku":["oauth-2025-04-20"]}`
	m := ParseClaudeCodeHTTPMimicryManifest(raw)
	require.NotNil(t, m)
	require.Equal(t, "2.1.153", m.CCVersion)
...
func TestParseClaudeCodeHTTPMimicryManifest_RejectsInvalid(t *testing.T) {
	cases := []string{
		...
		`{"schema_version":1,"cc_version":"2.1.153","sonnet_opus":[],"haiku":["oauth-2025-04-20"]}`,
	}
