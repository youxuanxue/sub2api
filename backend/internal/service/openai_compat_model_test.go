...
func TestForwardAsAnthropic_OAuthCompatCompactionKeepsAnchorsAndTailWhenConfigured(t *testing.T) {
	...
	guard := gjson.GetBytes(upstream.lastBody, "input.1.content.0.text").String()
	require.Contains(t, guard, "<sub2api-claude-code-todo-guard>")
	require.Contains(t, guard, "requested implementation and verification are complete")
...
func TestForwardAsAnthropic_AstraContinuationRestoresHistoryAndDisablesUnsupportedSession(t *testing.T) {
	for _, message := range []string{
		...
		t.Run(message, func(t *testing.T) {
			...
			for i, sent := range upstream.bodies[1:] {
				if i == 1 && strings.Contains(message, "not available for this user") {
					// Ownership mismatch replays once and then resumes continuation
					// against the response now owned by this account.
					require.Equal(t, "resp_replayed", gjson.GetBytes(sent, "previous_response_id").String())
