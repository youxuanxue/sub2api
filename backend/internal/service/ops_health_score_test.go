...
func TestComputeDashboardHealthScore_Comprehensive(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "good SLA with TTFT P99 spike only (prod-like)",
			overview: &OpsDashboardOverview{
				RequestCountTotal: 1000,
				SLA:               0.995,
				ErrorRate:         0.005,
				UpstreamErrorRate: 0.001,
				TTFT:              OpsPercentiles{P90: intPtr(2000), P99: intPtr(18707)},
				SystemMetrics: &OpsSystemMetricsSnapshot{
					DBOK:               boolPtr(true),
					RedisOK:            boolPtr(true),
		...
		},
		{
			...
			overview: &OpsDashboardOverview{
				...
				TTFT:              OpsPercentiles{P90: intPtr(600), P99: intPtr(600)},
				SystemMetrics: &OpsSystemMetricsSnapshot{
					DBOK:               boolPtr(true),
					RedisOK:            boolPtr(true),
		...
		},
		{
			...
			overview: &OpsDashboardOverview{
				...
				TTFT:              OpsPercentiles{P90: intPtr(8000), P99: intPtr(10000)},
				SystemMetrics: &OpsSystemMetricsSnapshot{
					DBOK:               boolPtr(true),
					RedisOK:            boolPtr(true),
	...
	tests := []struct {
	...
	}{
		{
			...
			overview: &OpsDashboardOverview{
				...
				ErrorRate:         0,
				UpstreamErrorRate: 0,
				TTFT:              OpsPercentiles{P90: intPtr(500), P99: intPtr(500)},
			},
			wantMin: 100,
			wantMax: 100,
		},
		...
		{
			name: "SLA 99.9% with high TTFT P99 only (P90 healthy)",
			overview: &OpsDashboardOverview{
				SLA:               0.999,
				ErrorRate:         0.001,
				UpstreamErrorRate: 0,
		...
		{
			name: "TTFT P90 boundary 10s",
			overview: &OpsDashboardOverview{
				SLA:               0.99,
				ErrorRate:         0,
				UpstreamErrorRate: 0,
				TTFT:              OpsPercentiles{P90: intPtr(10000), P99: intPtr(10000)},
			},
			wantMin: 60,
			wantMax: 65,
		},
		{
			name: "upstream error does not override SLA score",
			overview: &OpsDashboardOverview{
				SLA:               0.995,
				ErrorRate:         0.001,
