...
func TestOpenAI429FastPath_HTTPQuotaParkingPreservesResetBelowExhaustion(t *testing.T) {
	repo := &oauth429RateLimitRepo{}
	settingRepo := newMockSettingRepo()
	settingRepo.data[SettingKeyRateLimit429CooldownSettings] = `{"enabled":false,"cooldown_seconds":12}`
	rateLimitService := NewRateLimitService(repo, nil, &config.Config{}, nil, nil)
	...
	require.True(t, svc.isOpenAIAccountRuntimeBlocked(account), "real HTTP 429 retains TokenKey quota parking after retry exhaustion")
	block := svc.peekOpenAIAccountRuntimeBlock(account)
	require.True(t, block.blocked)
	require.Greater(t, time.Until(block.until), 6*24*time.Hour)
	require.Zero(t, repo.setRateLimitedCalls, "disabled 429 fallback must not persist a scheduler cooldown")
}
...
func TestOpenAI429FastPath_SparkQuotaOnlyBlocksSparkModel(t *testing.T) {
	...
	account := &Account{ID: 425, Platform: PlatformOpenAI, Type: AccountTypeOAuth}
	headers := http.Header{}
	headers.Set(codexActiveLimitHeader, codexBengalfoxActiveLimit)
	headers.Set("x-codex-primary-used-percent", "100")
	headers.Set("x-codex-primary-reset-after-seconds", "604800")
	headers.Set("x-codex-primary-window-minutes", "10080")
	headers.Set("x-codex-secondary-used-percent", "20")
...
func TestOpenAI429FastPath_SparkShadowQuotaRemainsProbeOwned(t *testing.T) {
	repo := &oauth429RateLimitRepo{}
	rateLimits := NewRateLimitService(repo, nil, &config.Config{}, nil, nil)
	svc := &OpenAIGatewayService{rateLimitService: rateLimits}
	rateLimits.SetAccountRuntimeBlocker(svc)
