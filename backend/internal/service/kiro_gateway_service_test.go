...
func TestUS041_KiroGatewayService_ClientCompletionNamedToolIsPreserved(t *testing.T) {
		...
		for _, claudeCode := range []bool{false, true} {
			t.Run(fmt.Sprintf("stream=%t/claude_code=%t", stream, claudeCode), func(t *testing.T) {
				...
				result, err := svc.Forward(context.Background(), c, newKiroAccountForTest(),
					newKiroToolRequestForTest(stream, claudeCode, "sub2apiClaudeCodeCompletion"), time.Now())
