...
func TestBuildUpstreamRequestNamespacesCodexIdentityByOAuthAccount(t *testing.T) {
	...
	for _, header := range []string{"x-codex-installation-id", "x-codex-window-id", "session-id", "thread-id", "x-client-request-id"} {
		require.Empty(t, first.Get(header), "headers outside the captured HTTP profile must remain excluded: %s", header)
	}
}
