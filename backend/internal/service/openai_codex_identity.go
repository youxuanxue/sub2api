...
// buildCodexCLIUserAgent updates every version-bearing segment while keeping
// the captured CLI OS and terminal fingerprint verbatim.
func buildCodexCLIUserAgent(version string) string {
	if version = NormalizeCodexClientVersion(version); version == "" {
		return codexCLIUserAgent
	}
	return codexCLIUserAgentPrefix + version + codexCLIUserAgentMiddle + version + codexCLIUserAgentSuffix
}
