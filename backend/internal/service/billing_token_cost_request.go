...
func (s *BillingService) CalculateTokenCostForRequest(req TokenCostRequest) (*CostBreakdown, error) {
	...
	if req.LongContextThreshold > 0 && (req.Group == nil || req.Group.LongContextPricingEnabled) {
		cost, err := s.CalculateCostWithLongContextAt(req.Model, req.Tokens, req.RateMultiplier, req.LongContextThreshold, req.LongContextMultiplier, RecordUsageBillingAt(req.PricingAt))
		if err == nil {
