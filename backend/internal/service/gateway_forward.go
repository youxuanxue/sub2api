...
func (s *GatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, parsed *ParsedRequest) (result *ForwardResult, err error) {
	...
	if result, handled, err := s.tkTryForwardKiro(ctx, c, account, parsed, startTime); handled {
		return result, err
	}
	...
	var isClaudeCode, shouldMimicClaudeCode bool
	reqModel, isClaudeCode, shouldMimicClaudeCode, err = s.tkApplyClaudeOAuthMimicry(
		ctx, c, account, parsed, reqModel, groupAdmitsNonCC, getBody, replaceBody,
	)
	if err != nil {
		return nil, err
	}
