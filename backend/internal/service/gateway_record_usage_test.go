...
func TestGatewayServiceRecordUsage_DeepSeekAccountStatsMirrorsConfiguredBilling(t *testing.T) {
		...
		for _, slot := range []struct {
			...
			t.Run(model.name+"/"+slot.name, func(t *testing.T) {
				...
				require.InDelta(t, customerTotal, *log.AccountStatsCost, 1e-12,
					"TokenKey account stats must preserve the configured billing mirror before catalog fallback")
			})
		}
	}
}
...
func TestGatewayServiceRecordUsage_PrefersServerRequestIDOverClientMarker(t *testing.T) {
	usageRepo := &openAIRecordUsageLogRepoStub{}
	billingRepo := &openAIRecordUsageBillingRepoStub{result: &UsageBillingApplyResult{Applied: true}}
	svc := newGatewayRecordUsageServiceWithBillingRepoForTest(usageRepo, billingRepo, &openAIRecordUsageUserRepoStub{}, &openAIRecordUsageSubRepoStub{})

	ctx := context.WithValue(context.Background(), ctxkey.ClientRequestID, "client-stable-123")
	ctx = context.WithValue(ctx, ctxkey.RequestID, "req-local-authoritative")
	err := svc.RecordUsage(ctx, &RecordUsageInput{
		Result: &ForwardResult{
	...
	require.NoError(t, err)
	require.NotNil(t, billingRepo.lastCmd)
	require.Equal(t, "local:req-local-authoritative", billingRepo.lastCmd.RequestID)
	require.NotNil(t, usageRepo.lastLog)
	require.Equal(t, "local:req-local-authoritative", usageRepo.lastLog.RequestID)
}
