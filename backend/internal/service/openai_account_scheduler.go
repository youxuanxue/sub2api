...
type OpenAIAccountScheduleRequest struct {
	...
	Platform                string // upstream compat alias for tests / legacy callers
	SessionHash             string
...
func (req OpenAIAccountScheduleRequest) schedulePlatform() string {
	if strings.TrimSpace(req.GroupPlatform) != "" {
		return req.GroupPlatform
	}
	if strings.TrimSpace(req.Platform) != "" {
		return NormalizeOpenAICompatiblePlatform(req.Platform)
	}
...
func (s *defaultOpenAIAccountScheduler) selectBySessionHash(
	...
	if shouldClearStickySession(account, req.RequestedModel) || !account.IsOpenAICompatPoolMember(req.schedulePlatform()) || !account.IsSchedulable() {
		clearBinding()
		return nil, false, 0, nil
	}
	...
	if s.service.concurrencyService != nil {
		...
		return attachSelectionProfitGate(ctx, waitPlan), false, 0, nil
	}
	return nil, false, 0, nil
}
...
func (s *defaultOpenAIAccountScheduler) tryAcquireOpenAISelectionOrderWithBudget(
	...
	for i := 0; i < len(selectionOrder); i++ {
		...
		platform := req.schedulePlatform()
		fresh := s.service.resolveFreshSchedulableOpenAIAccount(ctx, candidate.account, platform, req.RequestedModel, req.RequireCompact, req.RequiredCapability)
		...
		fresh = s.service.recheckSelectedOpenAIAccountFromDB(ctx, fresh, req.GroupID, platform, req.RequestedModel, req.RequireCompact, req.RequiredCapability)
		if fresh == nil || !s.isAccountTransportCompatible(fresh, req.RequiredTransport) || !s.isAccountRequestCompatible(ctx, fresh, req) {
			release(result)
			continue
		}
...
func (s *defaultOpenAIAccountScheduler) tryFallbackToWeightedSticky(
	...
	for _, accountID := range []int64{req.StickyPreviousAccountID, req.StickyAccountID} {
		...
		account = s.service.recheckSelectedOpenAIAccountFromDB(ctx, account, req.GroupID, req.schedulePlatform(), req.RequestedModel, req.RequireCompact, req.RequiredCapability)
...
func (s *defaultOpenAIAccountScheduler) selectByLoadBalance(
	...
	accounts, err := s.service.listSchedulableAccounts(ctx, req.GroupID, poolPlatform)
	if err != nil {
...
func (s *defaultOpenAIAccountScheduler) finishLoadBalanceSelectionFallback(
	...
	if len(attempt.selectionOrder) == 0 {
		return nil, candidateCount, topK, loadSkew, s.service.tkGroupUnsupportedModelRecordErr(req.GroupID, req.RequestedModel, openAICompatNoCandidateError(req.RequestedModel, req.GroupPlatform, attempt.compactBlocked, nil, req.ExcludedIDs, nil))
	}
	...
	for pass := 0; pass < passes; pass++ {
		...
		for _, candidate := range attempt.selectionOrder {
			...
			fresh := s.service.resolveFreshSchedulableOpenAIAccount(ctx, candidate.account, req.schedulePlatform(), req.RequestedModel, false, req.RequiredCapability)
			if fresh == nil || !s.isAccountTransportCompatible(fresh, req.RequiredTransport) || !s.isAccountRequestCompatible(ctx, fresh, req) {
				continue
			}
			if !s.consumeOpenAISelectionDBRecheck(budget) {
				return nil, candidateCount, topK, loadSkew, noAvailableOpenAISelectionError(req.RequestedModel, compactBlocked, req.schedulePlatform(), filterStats.summary("selection_order_exhausted"))
			}
			fresh = s.service.recheckSelectedOpenAIAccountFromDB(ctx, fresh, req.GroupID, req.schedulePlatform(), req.RequestedModel, false, req.RequiredCapability)
			if fresh == nil || !s.isAccountTransportCompatible(fresh, req.RequiredTransport) || !s.isAccountRequestCompatible(ctx, fresh, req) {
				continue
			}
			if req.RequireCompact && openAICompactSupportTier(fresh) == 0 {
...
func (s *OpenAIGatewayService) SelectAccountWithSchedulerForCapability(
	...
	for _, option := range options {
		...
		case bool:
			if boolIdx == 0 {
				previousResponseCanMove = v
			} else {
				useUpstreamTokenCost = v
			}
			boolIdx++
		case string:
...
func (s *OpenAIGatewayService) SelectAccountWithSchedulerForVideo(
	...
	return s.selectAccountWithScheduler(ctx, groupID, "", sessionHash, requestedModel, excludedIDs, requiredTransport, "", "", true, false, PlatformOpenAI, false, false)
}
...
func (s *OpenAIGatewayService) selectAccountWithScheduler(
	...
	selection, decision, err := s.selectAccountWithSchedulerOnce(ctx, groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, requiredCapability, requiredImageCapability, requiredVideoSupport, requireCompact, platform, previousResponseCanMove, useUpstreamTokenCost)
...
func (s *OpenAIGatewayService) selectLegacyAccountByPreviousResponse(
	...
	if !s.openAIAccountMatchesSchedulingGroup(ctx, account, groupID, platform) || !compatible || !scheduler.isAccountTransportCompatible(account, requiredTransport) {
...
func (s *OpenAIGatewayService) selectAccountWithSchedulerOnce(
	...
	if scheduler == nil {
		decision.Layer = openAIAccountScheduleLayerLoadBalance
		if selection, hit, err := s.selectLegacyAccountByPreviousResponse(ctx, groupID, previousResponseID, sessionHash, requestedModel, restrictionModel, excludedIDs, requiredTransport, requiredCapability, requiredImageCapability, requiredVideoSupport, requireCompact, groupPlatform); err != nil {
			return nil, decision, err
		} else if hit {
	...
	if stickyWeighted && previousResponseCanMove && strings.TrimSpace(previousResponseID) != "" && groupPlatform == PlatformOpenAI {
