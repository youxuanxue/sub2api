...
func (s *defaultOpenAIAccountScheduler) tryAcquireOpenAISelectionOrderWithBudget(
	...
	for i := 0; i < len(selectionOrder); i++ {
		...
		fresh := s.service.resolveFreshSchedulableOpenAIAccount(ctx, candidate.account, req.GroupPlatform, req.RequestedModel, req.RequireCompact, req.RequiredCapability)
		...
		fresh = s.service.recheckSelectedOpenAIAccountFromDB(ctx, fresh, req.GroupID, req.GroupPlatform, req.RequestedModel, req.RequireCompact, req.RequiredCapability)
		if fresh == nil || !s.isAccountTransportCompatible(fresh, req.RequiredTransport) || !s.isAccountRequestCompatible(ctx, fresh, req) {
			release(result)
			continue
		}
...
func (s *OpenAIGatewayService) SelectAccountWithScheduler(
	...
	return s.selectAccountWithScheduler(ctx, groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, "", "", false, requireCompact, PlatformOpenAI, false, false)
}
...
func (s *OpenAIGatewayService) SelectAccountWithSchedulerForCapability(
	...
	return s.selectAccountWithScheduler(ctx, groupID, previousResponseID, sessionHash, requestedModel, excludedIDs, requiredTransport, requiredCapability, "", false, requireCompact, platform, previousResponseCanMove, false)
}
