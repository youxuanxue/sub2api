...
func (s *defaultOpenAIAccountScheduler) isAccountRequestCompatibleReason(ctx context.Context, account *Account, req OpenAIAccountScheduleRequest) (bool, string) {
	...
	if req.RequiredVideoSupport && !accountSupportsOpenAIVideoCapability(account) {
		return false, "video_not_supported"
	}
	return true, ""
}
...
func (s *OpenAIGatewayService) SelectAccountWithSchedulerForVideo(
	...
	return s.selectAccountWithScheduler(ctx, groupID, "", sessionHash, requestedModel, excludedIDs, requiredTransport, "", "", true, false, PlatformOpenAI, false)
}
