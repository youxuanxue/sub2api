...
func (s *accountRepoStubForBulkUpdate) ListAllWithFilters(context.Context, string, string, string, string, int64, string, int) ([]Account, error) {
	return nil, nil
}

func (s *accountRepoStubForBulkUpdate) ListWithFilters(_ context.Context, params pagination.PaginationParams, platform, accountType, status, search string, groupID int64, privacyMode string, channelType int) ([]Account, *pagination.PaginationResult, error) {
	s.listCalled = true
	...
	s.lastListFilters.channelType = channelType
	if s.listErr != nil {
...
func TestAdminService_BulkUpdateAccounts_StripsManagedProbeKeysFromExtra(t *testing.T) {
	repo := &accountRepoStubForBulkUpdate{}
	svc := &adminServiceImpl{accountRepo: repo}
	schedulable := true
	...
	require.NotContains(t, repo.lastBulkUpdate.Extra, UpstreamBillingProbeEnabledExtraKey)
	require.NotContains(t, repo.lastBulkUpdate.Extra, UpstreamBillingRateSyncEnabledExtraKey)
	require.NotContains(t, repo.lastBulkUpdate.Extra, UpstreamBillingProbeExtraKey)
	require.Equal(t, true, repo.lastBulkUpdate.Extra["custom_flag"])
}
...
// bulkAccountRepoAnthropicSum embeds bulk stub and exposes a deterministic sum for operator sync tests.
type bulkAccountRepoAnthropicSum struct {
	accountRepoStubForBulkUpdate
	anthropicConcurrencySum int64
	anthropicConcurrencyErr error
}

func (s *bulkAccountRepoAnthropicSum) SumConcurrencyAnthropic(context.Context) (int64, error) {
...
func TestBulkUpdateAccounts_SyncAnthropicOperatorConcurrency(t *testing.T) {
	repo := &bulkAccountRepoAnthropicSum{anthropicConcurrencySum: 24}
