...
type SystemSettings struct {
	...
	AutoGenerateDefaultToken     bool    // 注册后自动签发一把 trial key，默认 true
	AutoGenerateDefaultTokenName string  // 自动签发 key 的名字，默认 "trial"
	PricingCatalogPublic         bool    // 公开 pricing 目录页开关，默认 true
	// PricedServingGateEnabled 是运行期「定了价才能上」准入闸的按平台启用集
	// （逗号分隔的平台集合，非 boolean；首发 "gemini"）。
