...
func (s *GatewayService) ForwardAsResponses(
	...
	isClaudeCode := false
	shouldMimicClaudeCode := account.IsOAuth() && !isClaudeCode && !account.IsAnthropicOAuthPassthroughEnabled()
...
func (s *GatewayService) handleResponsesBufferedStreamingResponse(
	...
	finalResp, err := s.tkResolveAnthropicBufferedAssembly(
		...
		"forward_as_responses buffered: read error",
	)
	if err != nil {
		return nil, err
	}
...
func appendRawJSON(existing json.RawMessage, fragment string) json.RawMessage {
	// Anthropic announces tool blocks with input:{} before streaming the real
	// JSON. The placeholder is not an argument fragment.
	if len(existing) == 0 || (fragment != "" && strings.TrimSpace(string(existing)) == "{}") {
