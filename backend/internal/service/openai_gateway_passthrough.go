...
func (s *OpenAIGatewayService) forwardOpenAIPassthrough(
	...
	var prepErr error
	body, reqStream, prepErr = s.tkPrepareOpenAIPassthroughRequestBody(ctx, c, account, body, reqModel, requestedModel, reqStream)
	if prepErr != nil {
		return nil, prepErr
	}
...
func (s *OpenAIGatewayService) buildUpstreamRequestOpenAIPassthrough(
	...
	if err := s.tkApplyOpenAIPassthroughUpstreamHeaders(ctx, c, req, account, body); err != nil {
		return nil, err
	}
...
func (s *OpenAIGatewayService) handleStreamingResponsePassthrough(
	...
	codexState := newTkCodexFailureStreamStateForPassthrough(account)
	...
	for documentScanner.Scan() {
		...
		if data, ok := extractOpenAISSEDataLine(line); ok {
			...
			if trimmedData != "[DONE]" {
				...
				line, dataBytes, trimmedData, restoreErr = tkRestorePassthroughSSEPayload(c, dataBytes, rawEventType, line)
				if restoreErr != nil {
		...
		// 必须继续读取，不适用提前结束。
		if (sawDone || sawTerminalEvent) && line == "" && (!codexState.enabled || !codexState.sawBareError) {
			break
		}
	}
