...
func NormalizeOpenAICompatiblePlatform(platform string) string {
	switch platform {
	case PlatformNewAPI, PlatformGrok, PlatformKimi, PlatformZhipu, PlatformDeepseek, PlatformMiniMax, PlatformOpenCodeGo:
		return platform
	default:
...
func (s *OpenAIGatewayService) selectAccountForModelWithExclusions(ctx context.Context, groupID *int64, platform string, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, stickyAccountID int64, requiredCapability OpenAIEndpointCapability, preferLowUpstreamRate bool, enforceChannelRestriction bool) (*Account, error) {
	account, _, err := s.selectAccountForModelWithExclusionsStickyHit(ctx, groupID, platform, sessionHash, requestedModel, excludedIDs, requireCompact, stickyAccountID, requiredCapability, preferLowUpstreamRate, enforceChannelRestriction)
	return account, err
}

func (s *OpenAIGatewayService) selectAccountForModelWithExclusionsStickyHit(ctx context.Context, groupID *int64, platform string, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, stickyAccountID int64, requiredCapability OpenAIEndpointCapability, preferLowUpstreamRate bool, enforceChannelRestriction bool) (*Account, bool, error) {
	...
	if enforceChannelRestriction {
		...
		if s.checkChannelPricingRestriction(ctx, groupID, restrictionModel) {
			...
			return nil, false, tkOpenAICompatChannelPricingRestrictionError(restrictionModel)
		}
	} else {
		requestedModel = CanonicalizeOpenAICompatRoutingModel(requestedModel)
	}
	...
	if selected == nil {
		if !gatewayProfitControlGateActive(ctx) && requestedModel != "" {
			...
			if compatErr != nil {
				return nil, false, compatErr
			}
		}
		return nil, false, noAvailableOpenAISelectionError(requestedModel, compactBlocked, platform, filterStats.summary(""))
	}
...
func (s *OpenAIGatewayService) selectAccountWithLoadAwareness(ctx context.Context, groupID *int64, platform string, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, requiredCapability OpenAIEndpointCapability, useUpstreamTokenCost bool) (*AccountSelectionResult, error) {
	...
	if s.concurrencyService == nil || !cfg.LoadBatchEnabled {
		account, stickyHit, err := s.selectAccountForModelWithExclusionsStickyHit(ctx, groupID, platform, sessionHash, requestedModel, excludedIDs, requireCompact, stickyAccountID, requiredCapability, preferLowUpstreamRate, false)
		if err != nil {
			return nil, err
		}
