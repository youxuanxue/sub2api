...
func TestHandleUpstreamError_OpenAIHTML403DoesNotPenalizeAccount(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.True(t, shouldDisable, "HTML 403 must still fail over")
...
func TestHandleUpstreamError_OpenAIHTML403RepeatedNeverEscalates(t *testing.T) {
	...
	for i := 0; i < openAI403DisableThreshold+2; i++ {
		require.True(t, h.handle(openAI403HTMLBody), "第 %d 次 HTML 403 仍需触发 failover", i+1)
	}
...
func TestHandleUpstreamError_HTML403OnOtherPlatformsUnchanged(t *testing.T) {
	repo := &rateLimitAccountRepoStub{}
	svc := NewRateLimitService(repo, nil, &config.Config{}, nil, nil)
	account := &Account{ID: 506, Platform: PlatformGemini, Type: AccountTypeAPIKey}
	...
	require.Equal(t, 1, repo.setErrorCalls, "非 OpenAI 平台保持原有 SetError 行为")
}
