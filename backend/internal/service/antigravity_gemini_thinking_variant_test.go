...
func TestResolveGeminiThinkingVariant(t *testing.T) {
	...
	tests := []struct {
		...
		{"no thinkingConfig: untouched", catalog, "gemini-3.8-flash", []byte(`{"contents":[]}`), "", false},
		...
		{"injected default variants are usable", map[string]string{
			"gemini-3.6-flash-low":  "gemini-3.6-flash-low",
			"gemini-3.6-flash-high": "gemini-3.6-flash-high",
		}, "gemini-3.6-flash", budget("1000"), "gemini-3.6-flash-low", true},
		...
		{"tiered alias serves as derivation base", map[string]string{
			"gemini-3.8-flash":      "gemini-3.8-flash-tiered",
			"gemini-3.8-flash-high": "gemini-3.8-flash-high",
		}, "gemini-3.8-flash", budget("-1"), "gemini-3.8-flash-high", true},
...
func TestGetMappedModelResolvesBareGeminiModelForAllEntrypoints(t *testing.T) {
	...
	t.Run("default entrypoint falls back to high", func(t *testing.T) {
		// 空映射账号：走 DefaultAntigravityModelMapping，裸名天然 wire 到 -high。
		require.Equal(t, "gemini-3.8-flash-high",
			svc.getMappedModel(&Account{Platform: PlatformAntigravity}, "gemini-3.8-flash"))
	})
	...
	t.Run("models/ prefix is handled", func(t *testing.T) {
		require.Equal(t, "gemini-3.8-flash-high",
			svc.getMappedModel(&Account{Platform: PlatformAntigravity}, "models/gemini-3.8-flash"))
	})
...
// 3.6 账号在携带 thinkingConfig 时按档位选变体；不携带时保留 bare→-tiered floor。
func TestResolveGeminiThinkingVariantPreservesTieredFloorWithoutPreference(t *testing.T) {
	account := newAntigravityAccountWithMapping(map[string]string{
	...
	// thinkingConfig 存在但没有 level/budget → explicit=false，不动 floor。
	got, matched := resolveGeminiThinkingVariant(account, "gemini-3.6-flash", []byte(`{"thinkingConfig":{}}`))
	require.False(t, matched)
	require.Empty(t, got)
	require.Equal(t, "gemini-3.6-flash-tiered", mapAntigravityModel(account, "gemini-3.6-flash"))

	// 显式 thinkingLevel 时按档位选变体。
	got, matched = resolveGeminiThinkingVariant(account, "gemini-3.6-flash", []byte(`{"generationConfig":{"thinkingConfig":{"thinkingLevel":"low"}}}`))
	require.True(t, matched)
	require.Equal(t, "gemini-3.6-flash-low", got)
}

// 非 tier 的映射（如图片别名）保持权威，不做变体推导。
...
// 显式 wire tier 变体直接映射，不进 derive 降级。
