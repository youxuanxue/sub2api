...
func (s *AntigravityGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte, isStickySession bool) (*ForwardResult, error) {
	...
	// Gemini-family models: Messages converter → ForwardGemini wire (generateContent → v1internal).
	// Claude-family models keep TransformClaudeToGemini.
	geminiBody, err := s.buildMessagesForwardUpstreamBody(ctx, body, &claudeReq, projectID, mappedModel, transformOpts)
	if err != nil {
	...
	if err != nil {
		...
		return nil, candidateTransportFailure(ctx, s.writeClaudeError(c, http.StatusBadGateway, "upstream_error", "Upstream request failed after retries"), err)
	}
	resp := result.resp
	...
	if resp.StatusCode >= 400 {
		...
		if resp.StatusCode == http.StatusBadRequest && isSignatureRelatedError(respBody) && s.settingService.IsSignatureRectifierEnabled(ctx) {
			...
			for _, stage := range retryStages {
				...
				retryGeminiBody, txErr := s.buildMessagesForwardUpstreamBody(ctx, nil, &retryClaudeReq, projectID, mappedModel, s.getClaudeTransformOptions(ctx))
				if txErr != nil {
					continue
				}
		...
		if resp.StatusCode == http.StatusBadRequest && respBody != nil && !isSignatureRelatedError(respBody) {
			...
			if isThinkingBudgetConstraintError(errMsg) && s.settingService.IsBudgetRectifierEnabled(ctx) {
				...
				if claudeReq.Thinking == nil || claudeReq.Thinking.Type != "adaptive" {
					...
					retryGeminiBody, txErr := s.buildMessagesForwardUpstreamBody(ctx, nil, &retryClaudeReq, projectID, mappedModel, transformOpts)
					if txErr == nil {
						retryResult, retryErr := s.antigravityRetryLoop(antigravityRetryLoopParams{
							ctx:             ctx,
							prefix:          prefix,
