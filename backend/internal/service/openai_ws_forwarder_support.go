...
func (s *OpenAIGatewayService) resolveAccountByPreviousResponseIDForCapability(
	...
	if requestedModel != "" && !accountAdmitsRequestedModelWithContext(ctx, account, requestedModel) {
		return 0, nil, "", nil
	}
	...
	if s.schedulerSnapshot != nil && s.accountRepo != nil {
		...
		if !s.openAIAccountMatchesSchedulingGroup(ctx, latest, groupID, PlatformOpenAI) {
			return 0, nil, "", nil
		}
		...
		if requestedModel != "" && !accountAdmitsRequestedModelWithContext(ctx, latest, requestedModel) {
			return 0, nil, "", nil
		}
