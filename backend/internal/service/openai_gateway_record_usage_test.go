...
func TestOpenAIGatewayServiceRecordUsage_DeepSeekAccountStatsMirrorsCustomerPricingWithUpstreamAttribution(t *testing.T) {
	for _, model := range []struct {
		...
		for _, slot := range []struct {
			...
			t.Run(model.name+"/"+slot.name, func(t *testing.T) {
				...
				require.InDelta(t, customerTotal, *log.AccountStatsCost, 1e-12,
					"account statistics mirror settled token pricing before the group multiplier")
			})
		}
	}
}
...
func TestOpenAIGatewayServiceRecordUsage_PrefersServerRequestIDOverClientMarker(t *testing.T) {
	usageRepo := &openAIRecordUsageLogRepoStub{}
	billingRepo := &openAIRecordUsageBillingRepoStub{result: &UsageBillingApplyResult{Applied: true}}
	userRepo := &openAIRecordUsageUserRepoStub{}
	subRepo := &openAIRecordUsageSubRepoStub{}
	...
	ctx = context.WithValue(ctx, ctxkey.RequestID, "openai-server-id")
	err := svc.RecordUsage(ctx, &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
	...
	require.NoError(t, err)
	require.NotNil(t, billingRepo.lastCmd)
	require.Equal(t, "local:openai-server-id", billingRepo.lastCmd.RequestID)
	require.NotNil(t, usageRepo.lastLog)
	require.Equal(t, "local:openai-server-id", usageRepo.lastLog.RequestID)
}
...
func TestOpenAIGatewayServiceRecordUsage_ImageOnlyUsageStillPersists(t *testing.T) {
	...
	err := svc.RecordUsage(context.Background(), &OpenAIRecordUsageInput{
		Result: &OpenAIForwardResult{
			...
			Model:      "gpt-image-2",
			Usage:      OpenAIUsage{ImageOutputTokens: 200},
			ImageCount: 2,
			ImageSize:  "1K",
			Duration:   time.Second,
		},
...
func TestGrokVideoBillingUsesSeparateVideoRateMultiplier(t *testing.T) {
	...
	require.Equal(t, int64(1), *usageRepo.lastLog.VideoDurationSeconds)
}
...
func TestOpenAIGatewayServiceRecordUsage_GrokVideoUsesDefaultRateCard(t *testing.T) {
	...
	require.Equal(t, 1, usageRepo.lastLog.VideoCount)
	require.NotNil(t, usageRepo.lastLog.VideoDurationSeconds)
	require.Equal(t, int64(VideoBillingDefaultDurationSeconds), *usageRepo.lastLog.VideoDurationSeconds)
}
