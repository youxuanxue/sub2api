...
func TestCompatibleImagesForwardGemini(t *testing.T) {
	...
	for _, kind := range []string{"generation", "json_edit", "multipart_edit", "composite_multipart_alias", "channel_mapping", "account_mapping"} {
		t.Run(kind, func(t *testing.T) {
			...
			if kind == "composite_multipart_alias" {
				// The candidate owner supplies the resolved alias to parsing and forwarding.
				ctx := context.WithValue(c.Request.Context(), candidateRequestContextKey{}, &CandidateRequest{current: &candidateExecutionPath{model: model}})
			...
			result, err := svc.ForwardImages(c.Request.Context(), c, &Account{ID: 1, Platform: PlatformOpenAI, Type: AccountTypeAPIKey, Credentials: credentials}, body, parsed, CandidateEffectiveModel(c.Request.Context(), channelModel))
			require.NoError(t, err)
			require.Equal(t, 1, result.ImageCount)
			require.Equal(t, model, result.UpstreamModel)
			require.Equal(t, CandidateEffectiveModel(c.Request.Context(), firstNonEmptyString(channelModel, parsed.Model)), result.Model)
