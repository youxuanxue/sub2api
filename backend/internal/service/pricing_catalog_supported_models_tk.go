...
// probe (ops/observability/run-probe.sh) when the served fleet changes.
// Gemini/Vertex was added as a third probed family (the google group on us6,
// see ops/pricing); its set is EMPTY until the first gemini probe, and an empty
// gemini set is treated as passthrough so landing the scaffolding causes no
// catalog/menu regression. antigravity is still unprobed (canonical fallback).
...
// the google group (us6). EMPTY until the first gemini probe; while empty, the
