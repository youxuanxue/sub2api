...
// getAnthropicErrorThreshold, getAnthropicErrorWindowMinutes,
// anthropicCooldownTierTTLMinutes, anthropicCooldownTierEscalationsWindowMinutes,
// IncrementAnthropicUpstreamErrorCount, IncrementAnthropicCooldownTier(,
// IncrementAnthropicCooldownTierEscalations, anthropic_cooldown_tier_escalation,
// anthropic_upstream_error_cooldown_write_skipped, 403_escalated_to_error,
// ResetAnthropicCooldownTier). Recovery hotpaths must keep
// s.ResetAnthropicUpstreamErrorCounter(ctx, accountID) below.
var _ = []any{
	...
	(*RateLimitService).getAnthropicErrorThreshold,
	(*RateLimitService).getAnthropicErrorWindowMinutes,
}
...
func isHTMLResponse(body []byte) bool {
	...
	return bytes.HasPrefix(lower, []byte("<!doctype html")) ||
		...
		bytes.Contains(lower[:min(len(lower), 256)], []byte("<html"))
}
...
func (s *RateLimitService) HandleUpstreamError(ctx context.Context, account *Account, statusCode int, headers http.Header, responseBody []byte, requestedModel ...string) (shouldDisable bool) {
	...
	return s.tkDispatchHandleUpstreamError(ctx, account, statusCode, headers, responseBody, requestedModel, customErrorCodesEnabled)
}
...
func (s *RateLimitService) handle429(ctx context.Context, account *Account, headers http.Header, responseBody []byte, requestedModel ...string) bool {
	...
	// Shadow quota is owned by QueryUsage, not the parent's Responses headers.
	if account.IsShadow() {
		return false
	}
	if len(requestedModel) > 0 && s.HandleOpenAICodexSparkRateLimit(ctx, account, requestedModel[0], http.StatusTooManyRequests, headers, responseBody) {
		return true
	}
	...
	if account != nil && isOpenAIOAuthAccount(account) && s.runtimeBlocker != nil {
		...
		if calculateOpenAI429ResetTime(headers) == nil && parseOpenAIRateLimitResetTime(responseBody) == nil {
	...
	if resetTimestamp == "" {
		...
		case PlatformOpenAI, PlatformNewAPI:
			...
			// NewAPI 5h/weekly/monthly prose ("It will reset at …") is not OpenAI-shaped;
			// parse + cool the requested model until reset before short fallback.
			if account.Platform == PlatformNewAPI && s.tkTryHandleNewAPIUsageWindow429(ctx, account, headers, responseBody, requestedModel...) {
				return true
			}
...
func calculateOpenAI429ResetTime(headers http.Header) *time.Time {
	...
	// handle429 fall through to the configurable short cooldown.
...
type anthropic429Result struct {
	...
	window        string     // "5h" / "7d" / "" when undetermined
}
...
// 返回值表示是否写了精确 Retry-After（调用方据此决定是否抑制阶梯）。
...
func (s *RateLimitService) UpdateSessionWindow(ctx context.Context, account *Account, headers http.Header) {
	...
	// 7d 家族（passive_usage_7d_* / passive_usage_7d_sonnet_* / passive_usage_7d_oi_*）
	// 属于独立的 7 天窗口，绝不能被 5h 滚动连带清空。
...
// HandleOpenAICodexSparkRateLimit isolates an explicitly identified Spark limit.
// Generic exhausted windows remain account-wide, even on a Spark request.
func (s *RateLimitService) HandleOpenAICodexSparkRateLimit(ctx context.Context, account *Account, requestedModel string, statusCode int, headers http.Header, responseBody []byte) bool {
	if s == nil || account == nil || s.accountRepo == nil || account.IsShadow() || statusCode != http.StatusTooManyRequests {
		return false
	}
	if !tkShouldOpenAICodex429BeModelScoped(account, headers, responseBody, requestedModel) || !account.ShouldHandleErrorCode(statusCode) {
		return false
	}
	...
	if resetUnix := parseOpenAIRateLimitResetTime(responseBody); resetUnix != nil {
		...
		resetAt = &reset
	} else if isCodexBengalfoxActiveLimit(headers) {
		// These generic headers mirror the explicitly selected Spark window.
		sparkHeaders := headers.Clone()
		sparkHeaders.Del(codexActiveLimitHeader)
		resetAt = calculateOpenAI429ResetTime(sparkHeaders)
	}
	now := time.Now()
	...
	return s.tkTryOpenAICodexModelScopedCooldown(ctx, account, requestedModel, *resetAt)
}
...
// TokenKey owner migration: retained from backend/internal/service/ratelimit_service_tk_model_not_found.go during upstream convergence.
