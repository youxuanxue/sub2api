...
func (s *SettingService) refreshCachedSettings(settings *SystemSettings) {
	...
	claudeCodeVersion := NormalizeClaudeCodeUserAgentVersion(settings.ClaudeCodeUserAgentVersion)
	if claudeCodeVersion == "" {
		claudeCodeVersion = GetDefaultClaudeCodeUserAgentVersion()
	}
	...
	// The effective Codex version may come from the sync-owned setting, so
	// invalidate and resolve it on the next read instead of recomputing here.
