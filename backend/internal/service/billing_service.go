...
func (s *BillingService) initFallbackPricing() {
	...
	// TK: Fable / Gemini / GPT / DeepSeek / GLM / domestic tables — see billing_service_tk_fallback_tables.go
	s.tkInitFallbackPricingTables()
...
func (s *BillingService) getFallbackPricing(model string) *ModelPricing {
	...
	// TK: Fable / Claude floor / Gemini family — see billing_service_tk_fallback_tables.go
	if p := s.tkResolveFallbackTableFamilyPricing(modelLower); p != nil {
		return p
	}

	// TK: DeepSeek / GLM / domestic LLM / Codex overlay — see billing_service_tk_fallback_overlay.go
	if p := s.tkResolveFallbackOverlayPricing(modelLower); p != nil {
		return p
	}
...
//
// A registry-declared alias is excluded: sharing that owner's price is a
...
func (s *BillingService) IsServedViaFamilyFloor(model string) bool {
	...
	if _, declared := tkPricingRegistryAliasOwner(lower); declared {
		return false
	}
