...
type UpdateSettingsRequest struct {
	...
	// PricedServingGateEnabled is the runtime priced-serving gate's per-platform
	// enabled SET (comma-separated, e.g. "gemini"). Optional pointer: nil = keep
	// previous persisted value, so a partial PUT cannot clobber the set to "" and
	// silently disable the gate (BLOCKER3). Empty string is a valid explicit value
	// (gate fully rolled back). See docs/approved/priced-or-it-doesnt-ship.md.
