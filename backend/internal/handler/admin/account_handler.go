...
func NewAccountHandler(
	...
	rpmCache service.RPMCache,
	tokenCacheInvalidator service.TokenCacheInvalidator,
	accountTierServices ...*service.AccountTierService,
) *AccountHandler {
	var accountTierService *service.AccountTierService
	if len(accountTierServices) > 0 {
		accountTierService = accountTierServices[0]
	}
...
func (h *AccountHandler) GetAvailableModels(c *gin.Context) {
	...
	// TK: platform model catalog dispatch — see account_handler_tk_available_models.go
	h.tkRespondAvailableModels(c, account)
}
