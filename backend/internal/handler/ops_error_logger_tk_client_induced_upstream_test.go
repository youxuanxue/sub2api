...
func TestClassifyOpsUpstreamClientInducedRejectionOwnedByClient(t *testing.T) {
	...
	t.Run("local Anthropic tool context guard is request/client owned", func(t *testing.T) {
		...
		phase, _, errorOwner, errorSource := classifyOpsErrorLog(c, "invalid_request_error", "Invalid Anthropic tool continuation", "", http.StatusBadRequest)
	...
	t.Run("openai chat/completions unsupported-model 400 (structured invalid_request_error body)", func(t *testing.T) {
		...
		phase, _, errorOwner, errorSource := classifyOpsErrorLog(c, errType, "Upstream request failed", "", http.StatusBadRequest)
	...
	t.Run("openai context window exceeded surfaced as upstream_error 502 owned by client", func(t *testing.T) {
		...
		phase, _, errorOwner, _ := classifyOpsErrorLog(c, "upstream_error", contextWindowMsg, "", http.StatusBadGateway)
	...
	t.Run("newapi qwen non-streaming thinking validation is client-owned", func(t *testing.T) {
		...
		phase, _, errorOwner, _ := classifyOpsErrorLog(c, "invalid_request_error",
...
func TestClassifyOpsRoutingCapacityWinsOverClientInduced(t *testing.T) {
	...
	phase, _, errorOwner, _ := classifyOpsErrorLog(c, "api_error",
