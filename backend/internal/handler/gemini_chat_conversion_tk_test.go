	...
	for _, platform := range []string{service.PlatformOpenAI, service.PlatformNewAPI} {
		for _, stream := range []bool{false, true} {
			t.Run(platform+map[bool]string{false: "/json", true: "/stream"}[stream], func(t *testing.T) {
				...
				require.False(t, geminiV1BetaRequestPlatformAllowed(c, key), "legacy group alone cannot authorize Gemini")
				_, err := api.UniversalResolver().PrepareCandidateIngress(c, key, service.ShapeGemini, path, "gpt-5.4", body, "")
				require.NoError(t, err)
				require.True(t, geminiV1BetaRequestPlatformAllowed(c, key), "authorized candidate ingress must reach its Gemini-to-Chat Plan")
				require.False(t, geminiV1BetaRequestPlatformAllowed(c, nil))
