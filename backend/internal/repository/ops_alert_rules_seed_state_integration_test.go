...
func TestSeededAlertRuleStateAfterMigrations(t *testing.T) {
	...
	require.Equal(t, "P2", clientRule.severity, "client failures must not page as P1")
	require.False(t, clientRule.notifyEmail, "client failures are an operational signal, not an email page")
