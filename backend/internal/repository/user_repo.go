...
func (r *userRepository) createUser(ctx context.Context, txClient *dbent.Client, userIn *service.User, guardEmailAlias bool, domainLimit string) error {
	...
	if guardEmailAlias {
		aliasExists, err := existsByEmailAliasWithClient(ctx, txClient, userIn.Email)
		if err != nil {
			return err
		}
		if aliasExists {
...
func (r *userRepository) Update(ctx context.Context, userIn *service.User, fields service.UserUpdateFields) error {
	...
	if fields.TrajExportEnabled {
		updateOp = updateOp.SetTrajExportEnabled(userIn.TrajExportEnabled)
	}
...
func (r *userRepository) DeductAvailableBalance(ctx context.Context, id int64, amount float64) (deducted float64, err error) {
	...
	`
		rows, qErr := clientFromContext(ctx, r.client).QueryContext(ctx, updateSQL, amount, id)
		if qErr != nil {
			return qErr
		}
		defer func() { _ = rows.Close() }()
		...
		if scanErr := rows.Scan(&deducted); scanErr != nil {
			return scanErr
		}
		return rows.Err()
	})
...
func (r *userRepository) AdjustBalance(ctx context.Context, id int64, delta float64) (service.BalanceChange, error) {
	...
	`
		got, ok, scanErr := scanBalanceChange(ctx, clientFromContext(ctx, r.client), updateSQL, delta, id)
		if scanErr != nil {
			return scanErr
		}
		if ok {
		...
		current, curErr := r.currentBalance(ctx, id)
		if curErr != nil {
			return curErr
		}
...
func (r *userRepository) SetBalance(ctx context.Context, id int64, value float64) (service.BalanceChange, error) {
	...
	`
		got, ok, scanErr := scanBalanceChange(ctx, clientFromContext(ctx, r.client), updateSQL, value, id)
		if scanErr != nil {
			return scanErr
		}
		if !ok {
