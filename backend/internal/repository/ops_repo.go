...
func (r *opsRepository) LookupDeletedKeyAudit(ctx context.Context, key string) (*service.DeletedKeyAuditResult, error) {
	...
	var result service.DeletedKeyAuditResult
	err := r.db.QueryRowContext(ctx, `
		...
		LIMIT 1
	`, key).Scan(&result.UserID, &result.KeyName)
	if err != nil {
...
func buildOpsErrorLogsWhere(filter *service.OpsErrorLogFilter) (string, []any) {
	...
	// View filter: errors = platform/provider faults; excluded = client faults.
	view := ""
	if filter != nil {
	...
	switch view {
	case "", "errors":
		clauses = append(clauses, "COALESCE(e.error_owner,'') IN ('provider','platform')")
	case "excluded":
