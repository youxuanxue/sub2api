...
func (s *UserRepoSuite) TestListWithFilters_Status() {
	...
	users, _, err := s.repo.ListWithFilters(s.ctx, pagination.PaginationParams{Page: 1, PageSize: 10}, service.UserListFilters{Status: domain.StatusActive})
	s.Require().NoError(err)
...
func (s *UserRepoSuite) TestListWithFilters_Role() {
	...
	users, _, err := s.repo.ListWithFilters(s.ctx, pagination.PaginationParams{Page: 1, PageSize: 10}, service.UserListFilters{Role: domain.RoleAdmin})
	s.Require().NoError(err)
	s.Require().Len(users, 1)
...
func (s *UserRepoSuite) TestListWithFilters_CombinedFilters() {
	...
	users, page, err := s.repo.ListWithFilters(s.ctx, pagination.PaginationParams{Page: 1, PageSize: 10}, service.UserListFilters{Status: domain.StatusActive, Role: domain.RoleAdmin, Search: "b@"})
	s.Require().NoError(err, "ListWithFilters")
...
func (s *UserRepoSuite) TestCRUD_And_Filters_And_AtomicUpdates() {
	...
	users, page, err := s.repo.ListWithFilters(s.ctx, params, service.UserListFilters{Status: domain.StatusActive, Role: domain.RoleAdmin, Search: "b@"})
	s.Require().NoError(err, "ListWithFilters")
	s.Require().Equal(int64(1), page.Total, "ListWithFilters total mismatch")
	s.Require().Len(users, 1, "ListWithFilters len mismatch")
