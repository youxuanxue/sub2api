...
// Falls back to global GetProxyURL() if the account has no per-account proxy.
...
func setPayloadProfileArnForAccount(payload *KiroPayload, account *Account) {
	if payload == nil {
		return
	}
...
// HTTPDoer is the minimal HTTP execution contract used by the Kiro API call
// path. It lets the TokenKey integration layer inject a TLS/proxy-aware client
// (matching Kiro's expected fingerprint and egress) without this vendor package
// depending on TokenKey's transport. *http.Client satisfies this interface.
...
func CallKiroAPIWithDoer(doer HTTPDoer, account *Account, payload *KiroPayload, callback *KiroStreamCallback) error {
	...
	if payloadJSON, err := json.Marshal(payload); err == nil {
		logDebugf("[KiroAPI] Request payload: %s", string(payloadJSON))
	}
	...
	for _, ep := range endpoints {
		...
		var httpDoer HTTPDoer = GetClientForProxy(ResolveAccountProxyURL(account))
		if doer != nil {
			httpDoer = doer
		}
		...
		if resp.StatusCode == 429 {
			resp.Body.Close()
			logWarnf("[KiroAPI] Endpoint %s quota exhausted (429), trying next...", ep.Name)
