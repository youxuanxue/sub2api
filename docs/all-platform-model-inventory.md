...
- **原生 servable allowlist**：以 `pricing_catalog_supported_models_tk.go` 的 Go map 与本节各平台清单为准；Gemini native pool 已退休，历史 7-id Gemini/Vertex catalog 仅用于定价/Vertex 语义，不能当 prod native capacity。
- **newapi 经账号 `model_mapping` 服务的策展长尾**：Qwen/DashScope 走运行库中配置的 `channel_type=17` 池（具体账号成员从 DB/admin 配置读取，manifest 里的 `served_on` 只是静态映射证据锚点），DeepSeek 以账号 39 为证据锚点，VolcEngine/doubao/seedream/seedance 以账号 7 为证据锚点（2026-06-23 Ark chat 19 个 200 已补 manifest；`doubao-seed-translation-250915` 400 不进清单）；GLM 服务也走同一类 DashScope/Qwen 池，BigModel 仅作为 GLM 官方定价来源。
...
| `advertised_dead` | 在 `DefaultModels` 但实测 502/404/403 | 客户能在 /v1/models 或菜单里选到打不通的模型。OpenAI 侧已改为统一 servable+priced fallback：2026-07-10 起 `codex-auto-review` 明确判定为内部能力，走 deprecated-model 400，不再展示；native OpenAI floor 与 `api.ainzy.net/v1` floor 分离，native 保留 live-proven 8 个（#1322 含 GPT-5.6 族），`gpt-5`/chat/pro/search/5.1/`gpt-5.4-pro` 这批 delta gate 403 的 priced rows 不进默认面。`gpt-image-*` 不进默认面。gemini-2.0-flash、gemini-3.x chat 仍按 project-scoped watchlist 管理 | servable-refresh 复测确认 200 则留，否则从可见面移除，并用同一 allowlist 闸 `DefaultModels` |
...
servable allowlist 以 `pricing_catalog_supported_models_tk.go` 为准（gofmt 字母序）：
...
servable allowlist 共 **8**（#1322 GPT-pro1 直连实测）：
...
- **2026-07-10 边界（#1322 更新）**：native OpenAI floor 与 `ops/pricing/examples/openai-oauth-proven.json` 对齐为上述 8 个实测可服务 id；`gpt-5.2*`、`codex-auto-review`、`gpt-5`、chat/pro/search/5.1、`gpt-5.4-pro` 暂不进入 public/menu/default catalog。可路由 legacy GPT-5 写法（如 `gpt-5.4-high`、`codex-mini-latest`、`gpt-5-chat-latest`）在账号选择前归一到上述 floor id；裸 `gpt-5.3`、`gpt-5.3-chat-latest`、`gpt-5.3-codex*`、`gpt-5-codex*` 均作为非展示 alias 指向 `gpt-5.3-codex-spark`，`gpt-5.5-pro` 作为非展示 alias 指向 `gpt-5.5`。裸 `gpt-5.6` 为兼容别名，经 wire transform 映射到 `gpt-5.6-sol`（直连上游裸名仍 400）。`gpt-5.3-chat-latest` 与 `gpt-5.5-pro` 于 2026-07-10 通过 edge-us4 account 7 / edge-us3 account 5 到达 ChatGPT Codex 上游，但官方名均返回 400 `not supported when using Codex with a ChatGPT account`，所以不展示、不进 floor。
- **`advertised_dead` 收敛结果**：`gpt-image-1`/`gpt-image-1.5`/`gpt-image-2`（原生 OAuth 结构性做不了图，需 `type=apikey` 账号）和未列入 native floor 的旧镜像残留不进入 /v1/models fallback 或 admin 默认候选。
...
**(a) Qwen（运行库 `channel_type=17` Ali/DashScope 池）+ DeepSeek（账号 39，ct=43，group 11）**
...
**(c) GLM：BigModel 定价，DashScope 服务（运行库 Qwen/DashScope 池，ct=17）**
...
| `advertised_dead` | openai | `gpt-image-{1,1.5,2}` 旧镜像残留 | 已收敛 | native OpenAI 与 Ainzy floor 已分离；未列入 native floor 的死项不再进默认可见面 |
...
| 17 | Ali | 8 | Qwen/DashScope 池；具体账号成员以运行库 DB/admin 配置为准 |
