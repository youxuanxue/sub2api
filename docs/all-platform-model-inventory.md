...
- **2026-07-10 边界（#1322 更新）**：native OpenAI floor 与 `ops/pricing/examples/openai-oauth-proven.json` / `model-surface-bundle.json` 对齐为 live-proven OAuth 集；`gpt-5.2*`、`gpt-5`、chat/pro/search/5.1、`gpt-5.4-pro` 暂不进入 public/menu/default catalog。`codex-auto-review` 于 2026-07-28 prod 上游探针 200 后纳入 allowlist + floor（#1489）。可路由 legacy GPT-5 写法（如 `gpt-5.4-high`、`codex-mini-latest`、`gpt-5-chat-latest`）在账号选择前归一到上述 floor id；裸 `gpt-5.3`、`gpt-5.3-chat-latest`、`gpt-5.3-codex*`、`gpt-5-codex*` 均作为非展示 alias 指向 `gpt-5.3-codex-spark`，`gpt-5.5-pro` 作为非展示 alias 指向 `gpt-5.5`。裸 `gpt-5.6` 为兼容别名，经 wire transform 映射到 `gpt-5.6-sol`（直连上游裸名仍 400）。`gpt-5.3-chat-latest` 与 `gpt-5.5-pro` 于 2026-07-10 通过 edge-us4 account 7 / edge-us3 account 5 到达 ChatGPT Codex 上游，但官方名均返回 400 `not supported when using Codex with a ChatGPT account`，所以不展示、不进 floor。
...
3. **deadlist**：已退役、替代明确或产品上明确不可选的 id 进入 `deadlist`，并由 apply gate 阻止新的 probe 结果把它们直接写回 allowlist。2026-07-10 起 `gpt-5.2` / `gpt-5.2-pro` 属于 deprecated-model gate；`gpt-5.3` / `gpt-5.3-chat-latest` / `gpt-5.3-codex*` / `gpt-5-codex*` / `gpt-5.5-pro` 属于非展示 alias，走 `skiplist` 而非 hard reject。
