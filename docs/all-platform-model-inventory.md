...
| `advertised_dead` | 在 `DefaultModels` 但实测 502/404/403 | 客户能在 /v1/models 或菜单里选到打不通的模型。OpenAI 侧已改为统一 servable+priced fallback：2026-07-10 起 `codex-auto-review` 明确判定为内部能力，走 deprecated-model 400，不再展示；native OpenAI floor 与 `api.ainzy.net/v1` floor 分离，native 只保留 live-proven 4 个，`gpt-5`/chat/pro/search/5.1/`gpt-5.4-pro` 这批 delta gate 403 的 priced rows 不进默认面。`gpt-image-*` 与未确认的 `gpt-5.6*` 同样不进默认面。gemini-2.0-flash、gemini-3.x chat 仍按 project-scoped watchlist 管理 | servable-refresh 复测确认 200 则留，否则从可见面移除，并用同一 allowlist 闸 `DefaultModels` |
...
| `advertised_dead` | openai | `gpt-image-{1,1.5,2}` `gpt-5.6*` 旧镜像残留 | 已收敛 | native OpenAI 与 Ainzy floor 已分离；未列入 native floor 的死项不再进默认可见面 |
