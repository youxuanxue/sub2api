...
>
> **快照日期**：2026-06-21 抓取。实测探针基线：claude/gpt 2026-06-05、gemini/Vertex 2026-06-09、antigravity 2026-06-13（见 `pricing_catalog_supported_models_tk.go` 头注）。**point-in-time 状态会过期**——带 `transient` 标记的条目必须按 §4 的 reprobe-watchlist 定期复核，不能当永久结论。
>
...
- **原生 servable allowlist 数量**：anthropic 8、openai 15、gemini 7、antigravity 5、grok 4（5 个 Go map）。
- **newapi 经账号 `model_mapping` 服务的策展长尾** ~40（qwen/deepseek 在账号 60/39，doubao/seedream/seedance/glm 在账号 7）。
...
| 类别 | 是什么 | 现状 | 处置 |
|---|---|---|---|
| `servable_unpriced`（chat） | 可服务但无价 → 计 `$0` 无扣额 | **会发 P0 告警，不是 silent**（`served_zero_cost` 探针）。主例：grok-4.x/3/2 chat（passthrough 可服务、xai mirror 价 grep=0）、antigravity `tab_flash_lite_preview` | 补**官方**核实价进 overlay → 加 allowlist；或从 defaults 移除。**不要给 chat 加 fail-closed 守卫**（见下） |
...
servable allowlist 共 **4**（与公开目录、overlay xai 同源）：
...
- **`servable_unpriced` P0（chat，本清单最大）**：**grok-4.x / grok-3 / grok-2-vision chat 全族**（含 `grok-4.3` = SuperGrok Heavy OAuth 默认、最典型请求）—— passthrough **可服务**、但 overlay 与镜像里 xai 价 **grep=0**，显式请求返 200 计 `$0`（P0 告警）。**刻意 unpriced**（2026 价未核实，遵循「a wrong price has no probe」），属 `policy`。补**核实价**后才加进 allowlist。
...
| kind | 平台 | 代表模型 | 危 | 处置 |
|---|---|---|---|---|
| `servable_unpriced_zero_cost_p0` | grok | grok-4.x/3/2 chat 全族（含 grok-4.3 默认）| **高** | 补核实价 → allowlist；或 leave_excluded（当前 policy）|
...
| newapi（未上架）| `kimi-k2.x`(ct25)、`qwq-32b`/`qwen-turbo`(ct17)、`glm-4.x`(ct26)、`MiniMax-M2.x`(ct35)、`deepseek-v4 -none/-max`(ct43) | never | 从未探过，标候选不标死 |
...
| 中 | ct=17/43 Ali/DeepSeek 未接 id | `qwen-turbo`/`qwq-32b`；deepseek-v4 `-none/-max`（实为 adaptor 追加的思考后缀别名，非独立模型）| |
