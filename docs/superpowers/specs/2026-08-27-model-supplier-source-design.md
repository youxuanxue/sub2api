...
新档位账号必须调用现有账号创建服务，并显式跳过默认组绑定，保持未分组。普通 `CreateAccount`
的既有失败契约不变。已有账号被接管时保留原账号组；以后仍由账号组管理服务绑定或解绑，供应源
同步不得覆盖。“供应源同步成功”只表示账号配置已收敛，不表示账号进入了任何特定账号组。
...
1. 创建缺少的档位账号，初始 `model_mapping={}`、`schedulable=false`，并显式跳过默认组绑定，保持
   未分组；
...
首批三个案例必须各自提供完整、准确的供应事实，才能进入同步：供应商、通道、规范化 endpoint、
可用 API Key、逐行 client/upstream 模型 ID、合法采购比例。信息不完整，或现有库存账号不符合
第 6 节窄匹配时，只保存事实或保留只读证据；不扩大已有账号匹配，不改账号网关调度。
...
两者合并到一个档位 3 账号；逐模型真实探测全部成功后才写账号。当前缺少生产 API Key，因此只能
保存，不能完成同步验收。补全真实 endpoint 与凭证后，再按同一对模型验收，不靠猜测已有账号。

### 9.2 FMGo

只录入 Seedance 显式双 ID，ratio 0.50。当前缺少 API endpoint 与凭证；即使补全，第一版仍必须
返回 `protocol_unsupported` 且不写账号。这不是通用前缀替换规则。

### 9.3 百度千帆

账号 90 的只读库存显示 `channel_type=46`，不符合第一版固定的 OpenAI Chat transport，因此首批
不接管，也不改账号 90 的 transport 或调度。缺少供应凭证也不能完成指纹匹配。若要接入，必须另
建完整供应源事实，不把库存差异当成可同步案例。
...
新账号保持未分组，不借用 `newapi-default`。普通账号创建失败契约不变。已有账号组不受同步影响。
scheduler 仍只读原有 priority；普通账号的 gateway 和缓存语义保持不变。受管账号仅通过现有 Chat
路由和现有协议能力表接入，不增加供应源专用调度或网关分支。
...
- 新账号经现有账号服务以空 mapping、不可调度、未分组状态创建，不读取或绑定账号组；普通账号
...
- 空档账号保留且不可调度；已有账号只按第 6 节窄条件复用，非 active 精确匹配只报冲突、不新建重复
  账号。
- 未支持协议失败封闭；佳杰、FMGo、百度案例在事实不完整或不匹配时保持 `not_run` /
  `protocol_unsupported` / 只读库存证据，要求补全准确信息，不扩大匹配或改网关调度。
