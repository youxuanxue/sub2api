...
TokenKey 内置支付系统，支持用户自助充值，无需部署独立的支付服务。
...
> **易支付服务商选择**：易支付是第三方聚合支付协议。请根据资金通道、结算币种、合规要求和可接受的运营风险选择服务商。TokenKey 不为特定聚合支付服务商背书。
>
> 支付渠道的安全性、稳定性、费率、结算条款及合规性请自行鉴别，本项目不对任何第三方支付服务商做担保。
...
如果你之前使用外部支付服务，现在可以迁移到内置支付：

### 主要差异

| 对比项 | 外部支付服务 | 内置支付 |
|--------|--------------|---------|
| 部署方式 | 独立服务与数据库 | 内置于 TokenKey，无需额外部署 |
| 支付方式 | 取决于外部服务 | EasyPay、支付宝、微信支付、Stripe |
| 配置方式 | 外部服务配置与回调 | TokenKey 管理后台内统一配置 |
| 充值对接 | 通过 Admin API 回调 | 内部直接处理，更可靠 |
| 订阅套餐 | 取决于外部服务 | 暂不支持（计划中） |
| 订单管理 | 独立管理界面 | 集成在 TokenKey 管理后台 |

### 迁移步骤

1. 在 TokenKey 管理后台启用支付并配置服务商
2. 更新 Webhook 回调地址为 TokenKey 的回调地址
3. 确认新订单通过内置支付正常处理
4. 保留仍需查询的历史记录后，停用外部支付服务

> **注意**：外部支付服务中的历史订单数据不会自动迁移。
