...
- 业务流程、代码调用链、`/v1/messages` 路径细节、OpenAI 分组 / 账号 / 模型映射配置见 `docs/flows/openai-gateway-flow.md`，本文档不重复
...
后续按 `docs/flows/openai-gateway-flow.md` 完成「加 OpenAI 账号 → group → API Key → cc switch」。
...
给修改代码的人的提醒：本部署方案对代码状态有如下耦合，触及对应模块时需同步评审本文档。代码事实的源头说明在 `docs/flows/openai-gateway-flow.md` 4.2，本表只列部署联动后果。
