...
Phase 1 / uk1 sample:
  ...
  扩展 deploy-stage0 或新增 deploy-edge-stage0，先支持 edge_id=uk1。
...
Phase 2 / first-batch replication:
  ...
  deploy workflow 接收 edge profile 矩阵，不复制脚本。
```
...
```text
deploy-stage0 支持 edge profile：create / deploy / upgrade / rollback / smoke。
