...
| Restricted Gemini Web native request capability | `gemini_web_request_tk.go` (`geminiWebSupportsRequest`) through `candidateSupportsRequest` | Local session accounts reject unsupported native Worker request shapes before selection and on refresh. Explicit prod Web relays enter `protocolrouter` with a native Gemini target; converter legality remains with `Plan`, while native Gemini body restrictions remain owned here. |
