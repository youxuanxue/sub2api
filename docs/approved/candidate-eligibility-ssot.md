...
| Restricted Gemini Web native request capability | `gemini_web_request_tk.go` (`geminiWebSupportsRequest`) through `candidateSupportsRequest` | Session accounts and explicitly declared Web relays reject unsupported request shapes before selection and on refresh. This stays in the existing native Gemini capability path; governed protocol legality remains with Plan. |
