...
当前实现与验收以第 12、13 节为准；第 14 节记录用户明确的扩号目标，尚未实现。
下文浏览器执行器和迁移方案保留为历史调查记录。
...
第一轮结束时提出的后续验证顺序是先恢复可确认的源会话基线，再按“单一执行器独占会话 → 更新后的 Cookie 原子保存 → 重启 → 原图复验”验证生命周期。迁移时须协调源浏览器与 edge 的刷新归属，不能把长期运行的两个独立刷新循环作为生产方案。

### 运营重新登录后的串行复核（13:55–13:59 UTC / 北京时间 21:55–21:59）

运营确认上一轮 483 确实退出，并完成重新登录。第二轮没有复用旧登录快照；从当前浏览器导出图片链相关 Google 域 Cookie，纯 HTTP 每次请求之后保存更新后的 jar，重建容器从该持久化 jar 恢复。两个执行器串行运行，每个阶段后都从 Mac 原浏览器后台重新请求 `/app` 验证登录态。全过程继续经 us4，无新图生成、无生产账号调度变更。

| 检查点（2026-09-20 UTC） | 结果 | 边界 |
| --- | --- | --- |
| 13:55:28 源会话基线 | HTTP 200，SNlM0e 存在 | 刚重新登录后的实际新请求，不依赖页面旧缓存 |
| 13:56:17 纯 HTTP 原图下载后回查 | 原图下载成功；Mac 源会话仍有 SNlM0e | Cookie + bootstrap + 原图 RPC，未启动 Chromium |
| 13:57:16 HTTP 容器重建后回查 | 从保存的 jar 恢复，原图再次下载成功；源会话仍正常 | 验证短时跨容器凭据使用，不等同长期 refresh |
| Linux Chromium 导入加载 | 初始 bootstrap 字段齐全，预期身份匹配；页面未出现原图下载按钮 | 确认 Cookie 能恢复初始页面身份，未取得原图 |
| 13:59:13 Chromium 阶段后回查 | Mac 后台新请求缺少 SNlM0e，已停止 Google 探测 | 运营随后确认 483 确实再次退出；不再主动重试 |

两次 HTTP 图片均为 image/jpeg、2816×1536、2,088,976 字节，SHA-256 与上文原图一致。原图 RPC + 三跳下载分别约 **1,126 ms / 983 ms**；含 bootstrap 分别约 **1,413 ms / 1,246 ms**，不含生成和对客编码。不能据短时样本推导长期可靠性。

本轮在 Chromium 前后的源 Cookie 对照中，SID、1PSID、1PSIDTS、SAPISID 等核心值仍保留；发生值变化的是 SIDCC 系列。它只说明不能用“文件没有复制过去”解释现象，不揭示 Google 服务端是否撤销或如何判定会话。

与第一轮相比，这次通过中间回查将问题范围缩小到 **Linux Chromium 导入后加载页面的阶段**。本轮实验脚本未主动调用 RotateCookies POST 或 RotateCookiesPage，因此不能把第一次的 401 当作本轮失效的起因。但 Chromium 自动发起的后台认证请求没有被完整分阶段捕获，仍不能排除它们，亦不能宣称已定位到某个 TLS 指纹、浏览器版本或设备绑定机制。

浏览器加载期间单次 `docker stats` 观察到约 **495.4 MiB / 640 MiB** 内存、**77.31%** CPU（配置限额 0.75 CPU），不是 RSS/PSS 峰值或 p95。原图没有完成交付，不能以该值承诺完整生图容量。测试容器均已退出，四个线上容器保持 healthy。

运营已确认第二轮也确实退出。结合逐阶段回查，通用 Linux Chromium 导入加载与退出有重复的时间关联，但仍不把未捕获的 Google 内部机制写成确定根因。

当前推荐收敛为：后续优先验证 **纯 HTTP 单一会话 owner 的自动续期**，暂停把同一会话导入通用 Linux Chromium 的实验。完整重新登录、浏览器内核匹配、设备状态迁移等替代方案均未获本轮证据支持，不用它们增加运营步骤或冒充已解决。

### 对方案的影响

- 低成本方向调整为：优先验证纯 HTTP 原图 RPC 与授权下载，浏览器保留为协议和会话恢复的候选执行器；暂不以两账号常驻 Chromium 为生产前提。
- 纯 HTTP 全尺寸下载已复现，且第二轮从持久化 jar 重建容器后下载成功；自动续期、长期可用性和新图生成闭环仍未通过本轮验证。此前 Mac 生图证据仍然有效，但不能替代新的容器端生成与交付验收。
- Linux Chromium 的完整迁移验收未通过：第二轮初始身份可恢复，但随后源会话新请求失去登录态。重启后原图下载、浏览器 refresh 后原图下载仍不能标记为通过。
...
## 14. 扩号目标：在目标 edge 后台复制账号，只替换浏览器凭证（待实现）

用户明确的运营入口是 TokenKey 管理后台：在不同 edge 上复制已有 Gemini Web 账号，
点击编辑，只更新该账号的凭证。凭证由运营从相应指纹浏览器复制，或导出文件后导入。
这替代上一轮建议的本机工具选择 profile、主动推送到 edge 的导入入口；
不要求运营接触服务器目录、Worker key、SSH、容器重启或额外账号绑定脚本。
本节是后续实施与验收目标，不代表 PR #2262 已具备后台导入能力。

### 运营流程与 edge 归属

1. 在 TokenKey 管理后台选择目标 edge，使用已有“在 Edge 管理”入口进入该 edge 的账号页。
   凭据编辑、创建、删除继续在目标 edge 执行，沿用现有边界，不让 Google Cookie 经 prod 中转或存储。
2. 复制该 edge 上已有的 Gemini Web 账号，继承通路、模型配置、分组、并发及调度参数。
...
各 edge 必须先具备 Worker 服务及可复制的账号模板；这是平台部署前置条件，不能让运营每加一个账号重复部署。
Worker 地址由目标 edge 的配置解析，跨 edge 复制配置不复制源 edge 的内部地址、分组 ID 或会话引用。
prod 已有该 edge 的中继且分组/模型权限匹配时，新增 edge 账号自动进入已有池，无需每号创建 prod 中继。
新 edge 尚无中继时，仍须先配置该 edge 的中继关系，复制账号不会凭空建立它。

### 运营凭证与内部密钥分离

运营输入的是浏览器会话包，内部建议结构如下（仅示意，非已发布导入协议）：

```json
{
  "format": "tokenkey-gemini-web-session-v1",
  "ua": "<该指纹浏览器实际 User-Agent>",
  "cookies": [
    {
      "name": "<cookie name>",
      "value": "<cookie value>",
...
必须保留 Cookie 的域、路径、过期时间及相关属性，UA 来自相同 profile。
浏览器原生导出若缺少 UA，应由导出辅助动作把实际 UA 与 Cookie 一次封装为完整凭证；
它只负责“复制/导出”，不承担创建 TokenKey 账号或向服务器上传的运营流程。
...
Worker 需要支持按账号热加载，新增/更新一个账号不应重启整个服务或打断其他账号生成。

### 当前缺口与验收边界

当前通用复制服务会复制 credentials 并将副本设为不可调度；Gemini Web 尚无专用凭证控件，
Worker 也只在启动时读取 accounts.json。直接把浏览器 Cookie 粘贴进现有 API Key 框不会完成绑定。
需要补齐 Gemini Web 的显式通路识别、复制时清理来源绑定、共享凭证编辑控件、目标 edge 会话安装接口、
Worker 热加载和验证状态。不能仅通过改账号名称或猜测 base URL 来识别通路。

新建和编辑必须复用同一凭证解析/保存 owner；Edge 管理与普通账号页复用已有账号编辑组件，
不在多个页面复制导入状态机。具体 owner 及安全/API 契约在实施时沿用项目审批与 SSOT 登记。

验收覆盖真实后台“复制 → 编辑 → 粘贴/文件导入 → 保存验证”，两个不同 Google 身份的文本与原图响应，
重复身份、错误/过期凭证、并发刷新期间换凭据、新副本未授权不得调度，以及更新一号不影响另一号。
跨 edge 的部署与导入尚未实测；本会话所有 Google 主动实测仍只能经 edge-us4 出口，
本节多 edge 产品目标不构成扩大当前测试出口的授权。
