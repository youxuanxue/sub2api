...
- 归档操作前必须通过 admin API 建立 cleanup hold：保留完整 advanced-settings，只将
  ...
  在控制端和源主机查询前各重验一次，正在运行或 hold 后才完成的 cleanup 均拒绝；hold
  已生效时重复 apply 也拒绝，避免覆盖 receipt 保存的原 enabled 状态；release 在恢复前
  必须再次证明同一 receipt 的 hold 仍有效，旧 receipt 不能释放后续 hold。
...
- 离机 staging 只使用 `tokenkey-stage0-backups` 的 `BucketName` 输出，key 固定在
  ...
  S3 必须报告服务端加密。
- 远端 Python bundle 先以确定性 tar.gz 上传到同一 staging 前缀的 `control/`，源主机下载后
  校验 SHA-256 再执行；SSM `commands` 只承载小型 loader 并有 20 KiB 参数硬上限，不内嵌源码。
...
plan (no AWS call)
  ...
  -> upload encrypted control bundle + verify checksum
  -> SSM host export
       -> reverify cleanup hold on source host
...
- cleanup hold 的 plan/apply/verify/release 有固定确认口令；apply 只改 cleanup bool 且拒绝覆盖已生效 hold，
  canary 缺 receipt、实例不匹配、当前 cleanup 已恢复或 hold 后又运行 cleanup 时均拒绝。
- 错误确认串、非 ops 表、远程/生产恢复目标、超量、热数据、时钟漂移和非只读会话均 fail closed。
- 多于 `max_rows` 的冷数据会产出确定性的首个有界样本，不会因存在后续行而拒绝；bigint
  `id` 使用保持数值顺序的规范编码，manifest 的数值首尾 key 与 continuation 状态必须通过 verify。
