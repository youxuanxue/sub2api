...
| US-030（自动建 trial key） | 邮箱注册 + OAuth 双路径都建 Key；setting 关闭时不建；建 Key 失败不阻塞注册成功；dashboard 快速开始卡片状态正确 | `go test -tags=unit ./internal/service/... -run TestUS030_` 与 `pnpm vitest run __tests__/components/UserDashboardQuickStart` |
...
- 写 `US-032-playground-experience.md` 故事档案，把 A 的 Storybook screenshot + B 的 HTML 路径作为 Evidence 归档
