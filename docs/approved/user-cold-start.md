...
authors: [agent]
created: 2026-04-22
related_stories: [US-027, US-028, US-029, US-030, US-031]
---
...
**本节只是方向陈述，不是落地规约。** Playground 的最终形态由 §11 的 prototype 审批决定。这里仅列出当前**倾向**的轮廓，供 prototype 阶段参考：

- 路由 `/playground`，sidebar 增加入口
- 顶部 group + model 下拉（model 来自 `/v1/models`，需要登录后用户自己的 trial key）
...
| 故事 | 主要 AC | 运行命令 |
| --- | --- | --- |
| US-027（公开 pricing） | 未登录可访 200；setting 关闭时返回 404；group_id 过滤生效；非 active group 不暴露 | `go test -tags=unit ./internal/handler/... -run TestUS027_` |
| US-028（注册赠额，USD） | 邮箱注册 + OAuth 双路径都加额；setting 关闭时不加额；setting=0 时不加额；system_log 正确写入；与 promo 不冲突 | `go test -tags=unit ./internal/service/... -run TestUS028_` |
| US-029（自动建 trial key） | 邮箱注册 + OAuth 双路径都建 Key；setting 关闭时不建；建 Key 失败不阻塞注册成功；dashboard 快速开始卡片状态正确 | `go test -tags=unit ./internal/service/... -run TestUS029_` 与 `pnpm vitest run __tests__/components/UserDashboardQuickStart` |
...
| Pricing 数据来源 (litellm JSON) 与实际可调模型脱节 | 中 | 公开页只显示 `gatewayService.GetAvailableModels(g)` 与 pricing JSON 取交集的模型；交集为空的模型隐藏 |
...
2. **Step 2**：实现 `signupBonusApplier` + `autoFirstKeyApplier` 两个内部小 helper，挂到 3 个注册路径（unit test US-028 + US-029）。
3. **Step 3**：实现 `pricingCatalogService.GetPublic()` + `GET /api/v1/public/pricing` 路由（unit test US-027）。
4. **Step 4**：前端 `/pricing` 页面 + dashboard 快速开始卡片（trial key 文案）+ AppHeader 公开导航。
...
按你的拍板，PR 2 的 prototype 阶段必须**同时**交付以下两件产物——A 用于工程对接（含 props/state 真实形态、Vitest/Playwright 可挂），B 用于直接给眼睛过目（无需启动任何服务）：

<!-- prettier-ignore -->
| 必交件 | 产物形态 | 角色 |
| --- | --- | --- |
| A. **Storybook story**（必做） | `frontend/src/components/playground/__stories__/PlaygroundView.stories.ts` 含 4 个 state：empty / typing / responded / error | 工程对接基线 — 后续实装直接复用同一 props/state 结构；CI 可跑 visual regression（如启用 chromatic 或本地 storybook screenshot） |
| B. **静态 HTML mockup**（必做） | `docs/approved/attachments/playground-prototype-2026-04-XX.html` 单页，CSS 内联，所有数据 hard-coded，不依赖任何后端或前端运行时 | 视觉决策基线 — 双击即可在浏览器打开，方便你 / 利益相关方在不启动 dev server 的情况下逐画面定稿 |
...
- 前端 `PlaygroundView.vue` + 子组件实装（基于 Storybook story 同形态 props/state，**不允许偏离**）
- 路由 `/playground` 注册 + sidebar 入口
- e2e Playwright 跑通 4 个画面（断言 DOM 结构与 Storybook story 一致）
- 写 `US-031-playground-experience.md` 故事档案，把 A 的 Storybook screenshot + B 的 HTML 路径作为 Evidence 归档

**不做 prototype（A + B 任缺一件）就直接写代码 = 违反本设计文档** = 这次 PR 1 的合并审批连带失效。这条门禁存在的意义是承认"Playground 的好坏只能看见，不能读出来"——而看必须在两个独立载体上能复现。
