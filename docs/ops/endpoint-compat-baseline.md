...
| Runtime code anchor | `v1.8.98` release (`backend/cmd/server/VERSION`); last live deploy `v1.8.97`. The 2026-07-05 focused Anthropic closeout also includes the live config remediation that set edge default Anthropic group `id=1` to `claude_code_only=false` on `us3/us4/us5/us6`. |
