...
| Runtime code anchor | `v1.8.84` live on `origin/main` @ `95299127f`; `backend/cmd/server/VERSION` = `1.8.84`. The 2026-07-05 focused Anthropic closeout also includes the live config remediation that set edge default Anthropic group `id=1` to `claude_code_only=false` on `us3/us4/us5/us6`. |
