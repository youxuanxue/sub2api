...
| Runtime code anchor | `v1.8.84` live on `origin/main` @ `4e0f8328f` (#1224–#1228: gateway Kiro stub model gating + gateway companion decomposition, auth/perf optimizations, storefront SEO prerender + /quickstart, upstream merge). Current PR removes Anthropic Fable/Opus 4.1 from the public client-facing allowlist until a later SSOT gate proves them `keep_displayed`. Prior anchor `v1.8.83` @ `000ffd756` (#1220–#1223). |
...
| Non-paid SSOT cleanup state | Latest focused 2026-07-05 `v1.8.84` live gate for the only remaining displayed Anthropic rows still has no `keep_displayed`: `claude-fable-5` and `claude-opus-4-1` chat = empty-pool `reprobe_required`, count_tokens = `503` transient `reprobe_required`, messages/responses = `hide_or_provision`; summary `DISPLAY_KEEP=0 DISPLAY_BLOCK=4 REPROBE_REQUIRED=4 FAIL=0`. OpenAI, Gemini, Grok, newapi, and Antigravity non-paid rows were already `keep_displayed` in the latest full closeout. Current PR product action: hide these two Anthropic SKUs from `supportedAnthropicCatalogModels` and add reprobe-ledger skiplist entries so public `/pricing`, Your Menu, and `/v1/models` no longer advertise them until a future SSOT gate returns `keep_displayed`. |
...
| `/tmp/tokenkey-ssot-gate-focused-anthropic-fable-opus-pr-20260705T082309Z.log` | Focused `v1.8.84` live gate for `claude-fable-5` and `claude-opus-4-1`: chat `429` empty-pool `reprobe_required`, count_tokens `503` transient `reprobe_required`, messages/responses `400` `hide_or_provision`; summary `DISPLAY_KEEP=0 DISPLAY_BLOCK=4 REPROBE_REQUIRED=4 FAIL=0 EXCLUDED_BLOCK=0 NO_ROWS=0`. This is the evidence for hiding those two public SKU rows in the current PR, not for deleting runtime pricing or native account support. |
...
| anthropic group 1 native API-key accounts | `claude-fable-5` and `claude-opus-4-1` across chat/messages/count_tokens/responses | open | Native prod/edge pool support is real when schedulable: prod `cc-us5` served both models on `/v1/messages`; edge us5 OAuth account 2 served both models on `/v1/messages` and `/v1/messages/count_tokens`; edge us6 OAuth account 14 served `claude-opus-4-1` on both paths. us6 `claude-fable-5` is currently model-level cooldown/empty-pool, not unsupported. | not public-display-safe on live `v1.8.84`: chat returns empty-pool `429`, count_tokens returns transient `503`, messages/responses return model/protocol-not-provisioned `400`; no focused row is `keep_displayed`. | account-model probe logs; latest sharded closeout; edge focused probe log; focused `v1.8.84` gate | Current PR hides these two rows from the public client-facing allowlist and guards the refresh path with skiplist entries. Re-display only after a non-throttled schedulable pool and the same universal SSOT gate return `keep_displayed`. |
...
| public `/pricing` SSOT projection | all derived model/protocol rows | n/a | n/a | latest deployed projection still includes Anthropic Fable/Opus 4.1 and the focused `v1.8.84` gate shows zero `keep_displayed` rows for them. Paid media focused full gate: 19 keep, 0 block/fail. | latest SSOT list; latest sharded closeout; latest paid media gate; focused `v1.8.84` gate | Current PR removes the last two non-paid Anthropic blockers from the public projection. After deploy, focused `--model claude-fable-5 --model claude-opus-4-1` should return no public rows, and the full non-paid gate should have no remaining Anthropic Fable/Opus display blockers. |
...
1. Current 2026-07-05 `v1.8.84` focused gate is the product action list for the last two Anthropic blockers. Grok `/v1/messages`, all newapi `/v1/responses`, OpenAI, Gemini, and Antigravity rows are resolved. Remaining current-PR action is to hide Anthropic `claude-fable-5` and `claude-opus-4-1` from the public client-facing allowlist/menus until the same universal SSOT gate returns `keep_displayed`.
   After this PR deploys, rerun:
   `python3 ops/test/gateway_model_ssot_matrix.py gate --show-excluded --model claude-fable-5 --model claude-opus-4-1`
   and expect no public rows for those two models. Re-display requires a non-throttled schedulable native pool plus `keep_displayed` for the relevant protocols; account-level probes alone are insufficient.
...
4. Current non-paid `reprobe_required` rows are only Anthropic Fable/Opus 4.1
   rows that this PR hides from public display. Reprobe after capacity/cooldown
   changes; do not count these as defects while the direct pool is empty.
