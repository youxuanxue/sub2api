...
| Non-paid SSOT cleanup state | Latest 2026-07-05 `v1.8.83` live `--gate-sharded --deploy-closeout` result: `DISPLAY_KEEP=348 DISPLAY_BLOCK=6 REPROBE_REQUIRED=2 FAIL=2 EXCLUDED_BLOCK=0`. OpenAI, Gemini, Grok, newapi, and Antigravity non-paid rows are all `keep_displayed`. Remaining rows are only Anthropic `claude-fable-5` / `claude-opus-4-1`: chat = empty-pool `reprobe_required`, messages/responses = `hide_or_provision`, count_tokens = `400 upstream_error` `hide_or_fix_gateway`. Same-day edge account probes show the underlying edge OAuth pool can serve count_tokens where the model is not on model-level cooldown, so the live prod count_tokens `FAIL` is a gateway/account-selection issue, not proof of upstream count_tokens absence. |
...
| anthropic group 1 native API-key accounts | `claude-fable-5` and `claude-opus-4-1` across chat/messages/count_tokens/responses | open | Native prod/edge pool support is real when schedulable: prod `cc-us5` served both models on `/v1/messages`; edge us5 OAuth account 2 served both models on `/v1/messages` and `/v1/messages/count_tokens`; edge us6 OAuth account 14 served `claude-opus-4-1` on both paths. us6 `claude-fable-5` is currently model-level cooldown/empty-pool, not unsupported. | not fully display-safe post-`v1.8.83`: chat returns empty-pool `429`, messages/responses return model/protocol-not-provisioned `400`, count_tokens returns gateway `400 upstream_error` because prod universal can fall through to the wrong mirror/stub class when the native pool is unavailable or filtered. | account-model probe logs; latest sharded closeout; edge focused probe log | Keep these rows hidden/reprobe-gated until the native schedulable pool is available and the gateway account-selection fix is deployed. Do not use Kiro mirror stubs as fallback for native-only Fable/Opus 4.1. |
...
1. Latest 2026-07-05 `v1.8.83` sharded closeout is the current non-paid product action list (`348 keep / 6 block / 2 reprobe / 2 fail`).
   ...
   - count_tokens rows are true gateway `FAIL` (`400 upstream_error`) on live `v1.8.83`; edge OAuth probes prove count_tokens itself works where the native model pool is schedulable. Fix account selection so native-only Fable/Opus 4.1 never fall through to Kiro mirror stubs, then focused-reprobe these two rows after deploy.
