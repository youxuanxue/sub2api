...
| **Vertex AI**（Service Account，走 Cloud Billing） | ✅ **吃** | 平台 Gemini + 类别 Vertex（service_account） |
...
3. 把第一步下载的 **JSON 文件拖进上传框**（或点选择文件）——会自动填出 **Project ID** 和 **Client Email**。
4. **Location** 选 `us-central1`（imagen / veo 都可用；不确定就用它）。
5. 保存。
...
| imagen/veo 被拒 | 分组没开「允许图片生成」，或账号没挂进该分组 |
...
> （`service_account_json` / `location`，project_id 由 JSON 自解析）；后台表单见
> `frontend/src/components/account/CreateAccountModal.vue` 的 Vertex Service Account 区；
> 分组出图门禁见 `group_service.go` 的 `AllowImageGeneration`。字段或门禁变动时同步更新此文档。
