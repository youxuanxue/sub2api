...
| 2.1.193 | 2026-06-25 | pure UA | 2.1.179 → 2.1.193. TLS ja3 unchanged; stainless unchanged; sonnet/opus beta unchanged; system identity anchor OK; Haiku A/B bimodal 8/11 vs 3/11, baseline matches a variant (per #429). Capture egress 3.148.79.145. |
| 2.1.195 | 2026-06-27 | pure UA | 2.1.193 → 2.1.195. TLS ja3 unchanged; stainless unchanged; sonnet/opus beta unchanged; system identity anchor OK; Haiku A/B bimodal 8/11 vs 3/11, baseline matches a variant (per #429). Capture egress 3.148.79.145. |
| 2.1.195 | 2026-06-29 | **decision** | Revert mistaken `X-Stainless-Package-Version` 0.106.0 → **0.94.0** (client-release-watch #1063 over-bump; mitm/TLS ground truth on cc 2.1.195). TLS ja3/beta/UA unchanged. Bundle: `.tls_list/20260629T080940Z-cc-capture.bundle.json` + `.tls_list/20260627T020627Z-cc-capture.bundle.json`. |
...
| 2.1.201 | 2026-07-05 | pure UA | 2.1.199 → 2.1.201 (no 2.1.200 release observed). TLS ja3 unchanged; stainless **0.94.0** unchanged (TLS capture; client-release-watch npm `@anthropic-ai/sdk@0.110.0` is not on-wire ground truth per 6/29 decision). HTTP mitm skipped (OAuth overlay expired). Bundle: `.tls_list/20260705T081838Z-cc-capture.bundle.json`. Capture egress 3.148.79.145. |
| 2.1.202 | 2026-07-06 | pure UA + REPL cohort | 2.1.201 → 2.1.202. TLS ja3 unchanged; stainless **0.94.0** unchanged. Interactive REPL capture (`capture-cc-interactive.sh`): HTTP UA `claude-cli/2.1.202 (external, cli)` ×6; TLS bare `-p` still `(external, sdk-cli)` on same patch. Haiku beta bimodal in REPL (claude-code-20250219 vs structured-outputs-2025-12-15) — baseline unchanged (#429). Bundle: `.tls_list/20260706T234304Z-cc-interactive.bundle.json`. PR #1253 canonical suffix cli pin. |
| 2.1.202 | 2026-07-07 | **decision** | cc 2.1.202 binary dropped minified `KSe(`/`eca(` substrings (2.1.197-era canaries, not geo semantics). `scripts/sentinels/cc-geo-stego-static.json` now anchors `The date has changed. Today's date is now` + `date_change:` wire surfaces instead. |
| 2.1.205 | 2026-07-09 | pure UA | 2.1.202 → 2.1.205. TLS ja3 unchanged; stainless **0.94.0** unchanged (TLS `-p` capture via `capture-cc-fingerprint.sh`; bundle `tls.user_agent` still records `(external, sdk-cli)` — informational only, not canonical egress). Canonical egress / mimic pins remain `(external, cli)` (PR #1253). HTTP mitm skipped — Claude Desktop not running. Bundle: `.tls_list/20260709T061548Z-cc-capture.bundle.json`. Closes client-release-watch #1280 / #1268. |
| — | 2026-07-09 | **decision** | OAuth mimicry **fingerprint scope** expanded: TLS + HTTP headers (beta, stainless, UA) + system billing/identity anchors — not UA-only. Added `gateway.anthropic_oauth_mimic_egress` log + `probe-oauth-mimicry-chain.sh`. edge-us3 probe: 641/800 ingress `OpenAI/Python` oauth rows, pre-deploy zero egress log lines. Record: [`spec-delta-cc-oauth-mimicry-fingerprint-scope.md`](spec-delta-cc-oauth-mimicry-fingerprint-scope.md). |
| 2.1.210 | 2026-07-15 | pure UA + 3p REPL cohort + passive pcap | 2.1.206 → 2.1.210. Native `claude` with the configured 3p token captured UA `claude-cli/2.1.210 (external, cli)` ×6; Stainless `0.94.0` / Node `v26.3.0` and system identity anchors remain aligned. OAuth beta was not used as evidence: the 3p path omits `oauth-2025-04-20` by design. After the TLS collector failed, the interactive mitm loopback pcap fallback captured the real ClientHello; JA3 hash/raw both match `tk_canonical_cc_oauth`, so the TLS profile remains unchanged. Bundles: `.tls_list/20260715T140530Z-cc-interactive.bundle.json` + `.tls_list/20260715T224911Z-cc-interactive.bundle.json`. |
| 2.1.211 | 2026-07-16 | pure UA + 3p REPL cohort + passive pcap | 2.1.210 → 2.1.211. Native `claude` with the configured 3p token captured UA `claude-cli/2.1.211 (external, cli)`; Stainless `0.94.0` / Node `v26.3.0` and system identity anchors remain aligned. The comprehensive matrix observed two 3p beta variants in each model family (Haiku 6, Sonnet 6, Opus 4 requests), so OAuth beta remains unchanged and the 3p variants are not used as OAuth evidence. After the TLS collector returned 502, the interactive mitm loopback pcap captured the real ClientHello; JA3 hash/raw both match `tk_canonical_cc_oauth`, so the TLS profile remains unchanged. Bundle/log: `.tls_list/20260716T034539Z-cc-interactive.bundle.json` + `.tls_list/20260716T024826Z-http-multi.log`. |
