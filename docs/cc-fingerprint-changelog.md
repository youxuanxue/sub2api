...
| 2.1.210 | 2026-07-15 | pure UA + 3p REPL cohort | 2.1.206 → 2.1.210. Native `claude` with the configured 3p token captured UA `claude-cli/2.1.210 (external, cli)` ×6; Stainless `0.94.0` / Node `v26.3.0` and system identity anchors remain aligned. OAuth beta and TLS were not used as evidence: 3p auth omits the OAuth beta by design, and the TLS collector was unavailable. Bundle: `.tls_list/20260715T140530Z-cc-interactive.bundle.json`. |
