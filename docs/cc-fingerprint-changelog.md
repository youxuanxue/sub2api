...
| 2.1.211 | 2026-07-16 | pure UA + 3p REPL cohort | 2.1.210 → 2.1.211. Native `claude` with the configured 3p token captured UA `claude-cli/2.1.211 (external, cli)`; Stainless `0.94.0` / Node `v26.3.0` and system identity anchors remain aligned. The comprehensive matrix observed two 3p beta variants in each model family (Haiku 6, Sonnet 6, Opus 4 requests), so OAuth beta remains unchanged and the 3p variants are not used as OAuth evidence. TLS is `NOT_OBSERVED`: the collector returned 502 and non-interactive pcap was unavailable, so the existing JA3 baseline is retained without claiming 2.1.211 TLS alignment. Bundle/log: `.tls_list/20260716T024307Z-cc-interactive.bundle.json` + `.tls_list/20260716T024826Z-http-multi.log`. |
