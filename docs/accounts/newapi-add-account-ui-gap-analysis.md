...
- 本文档使用 `status: draft`，等到对应修复 PR 合并时翻 `shipped` 并补 `related_prs / shipped_at / related_commits`。
- 如果这条 backlog 决定**不修**或**改方向**，请把本文档翻 `archived` 并写明原因（不要静默删除——dev-rules `safe-shell-commands` 与 OPC 哲学要求保留判断链）。
