...
- 状态: pending（诊断 + 修复方案均已提交；等待人工审批 + PR 合并后翻 shipped）
- 作者: agent
- 创建日期: 2026-04-23
...
- 本次修复（D1-D4 实现）: PR #47（含 8 个新 vitest 用例）
...
| `frontend/src/components/account/CreateAccountModal.vue` | watcher B 加 `newapi` 分支自动 `accountCategory='apikey'`；通用 apikey 块 v-if 增加 `&& form.platform !== 'newapi'`；新增 `newapiAllowed/Mapping/RestrictionMode/FetchLoading/LastUpstreamModels` 5 个 ref + `newapiFetchEnabled/Disabled` 2 个 computed + `handleFetchNewapiUpstreamModels()` handler；submit 路径用 `buildModelMappingObject` 替代旧 raw JSON parse；`watch(() => newapiChannelType)` 自动 prefill base_url；resetForm 增加 newapi 全状态清零 | D1 + D2 + D3 + D4 |
| `frontend/src/components/account/EditAccountModal.vue` | 通用 apikey 模型限制块 v-if 增加 `&& account.platform !== 'newapi'`；同样新增 `newapiAllowed/Mapping/RestrictionMode/FetchLoading` 4 个 ref + 2 个 computed + handler；syncFormFromAccount 在 newapi 分支按 whitelist/mapping 推断填充 selector；submit 路径换为 `buildModelMappingObject`；fetch 时支持空 api_key + account_id 路径（编辑账号时不必重新输入密钥） | D2 + D3 + D4 |
...
1. **`useTkAccountNewApiPlatform` composable 仍未启用**：原计划在 §3-D3 把 Create/Edit Modal 的 `listChannelTypes()` 直连方式整体替换为 composable。最终实现采用了**最小侵入**版本：保留两个 modal 各自的 `listChannelTypes()`/`newapiChannelTypes` ref + computed（约 ~30 行 × 2），仅在 modal 内部新增 `handleFetchNewapiUpstreamModels()` 直接调用 `fetchUpstreamModels()` API。理由：composable 的 `bootstrapNewapiCatalog()` + reactive `form.channel_type` 绑定要求重写两个 modal 的 reactivity 边界（form 不是 reactive object 而是 reactive() 包裹的状态），改动面 ~80 行 vs 收益主要是去重——**死代码下次 stage 自然会清掉**。本次主要目标是闭合 prod 体验缺口，先求生效再求合并。
...
4. **「测试连接」未在本 PR 范围**：上游 new-api 渠道还有「测试」按钮（POST /api/channel/test/{id}）。TK 已经有独立的 `AccountTestModal`（账号列表行的「⋮」菜单进入），UX 等价但路径不同。本次不动。
...
| AC-2 (D3 channel-type 自动 prefill) | ✅ | `CreateAccountModal.vue:3618-3636` watcher 实装；手测：选 channel_type=14 → base_url 自动填 `https://api.deepseek.com` |
...
| 回归: EditAccountModal 编辑已有 newapi 账号 | ✅ | `syncFormFromAccount` 中按 whitelist/mapping 推断模式后 prefill `newapiAllowed/Mapping`；fetch 按钮在编辑模式可不输入 api_key（走 account_id 路径） |
...
- 本文档使用 `status: pending`，PR #47 合并时翻 `shipped` 并补 `related_commits`。
- 如果这条 backlog 后续决定**改方向**（例如重启 `useTkAccountNewApiPlatform` composable 接入），请保留本文档作为决策链而不是覆盖；新增 `docs/accounts/newapi-add-account-ui-followup-*.md` 续写即可。
