...
上线后默认生效（billing_tier=`kiro-cache-estimated` 跟随开关本身，即使本请求无法拆分前缀也不回落 `kiro-estimated`；不再计入 dashboard 的 cache_telemetry_unavailable）。异常时将 `gateway.kiro_cache_billing.enabled=false`（回退 `kiro-estimated`）即可回滚。
