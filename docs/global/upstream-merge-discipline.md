...
| `.github/workflows/upstream-merge-notify.yml` | daily 04:00 Asia/Shanghai + manual dispatch | Detection-only: `check-drift.sh` + read-only `merge-tree`. When TokenKey is behind `upstream/main`, create or refresh one `[upstream-merge]` tracking issue (labels are best-effort; issue API uses `github.token` with job-level `issues: write`). Never merges, never opens a PR, never runs an agent. |
