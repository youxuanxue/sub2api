...
describe('resolveAvailableModels (transparent model picker)', () => {
  ...
  it('resolves a model + price via an alias id and reports the served id', () => {
    ...
    expect(out[0].presentation.modelId).toBe('seedream-4-0-250828')
  ...
  it('surfaces newly probed Seedream image models with curated presentation', () => {
    ...
    expect(out.map((r) => r.presentation.displayName)).toEqual(['Seedream 5.0', 'Seedream 4.5'])
  ...
  it('uses the routed doubao Seedance id as canonical and treats no-prefix as an alias', () => {
    ...
    expect(out).toHaveLength(1)
    expect(out[0].presentation.modelId).toBe('doubao-seedance-1-0-pro-250528')
    expect(out[0].servedId).toBe('doubao-seedance-1-0-pro-250528')
    expect(out[0].presentation.aliasIds).toContain('seedance-1-0-pro-250528')
  })
  ...
  it('surfaces Seedance Pro Fast with the conservative probed duration', () => {
    ...
    expect(out).toHaveLength(1)
    expect(out[0].presentation.modelId).toBe('doubao-seedance-1-0-pro-fast-251015')
    expect(out[0].presentation.displayName).toBe('Seedance 1.0 · Pro Fast')
...
describe('defaultModelId', () => {
  ...
  it('never auto-selects a needsApikeyAccount model', () => {
    // No shipped presentation is flagged today; assert the invariant holds for the overlay.
    expect(MEDIA_MODEL_PRESENTATIONS.some((m) => m.needsApikeyAccount)).toBe(false)
  })
})
...
describe('capability map honesty (verified against new-api adaptors)', () => {
  ...
  const byId = (id: string) => MEDIA_MODEL_PRESENTATIONS.find((m) => m.modelId === id)!

  it('every supportedParams entry is a valid StudioParam', () => {
    for (const m of MEDIA_MODEL_PRESENTATIONS) {
...
describe('video durations (per-model discrete, never a footgun)', () => {
  ...
  const videoModels = MEDIA_MODEL_PRESENTATIONS.filter((m) => m.modality === 'video')
  const byId = (id: string) => MEDIA_MODEL_PRESENTATIONS.find((m) => m.modelId === id)!
...
describe('image aspect options (per-model, upstream-valid wire values)', () => {
  ...
  it('gemini is flatImageBilling; imagen is flat-priced (no size tier); only seedream stays tiered', () => {
    ...
    for (const m of MEDIA_MODEL_PRESENTATIONS.filter((m) => m.imageSizes === SEEDREAM_IMAGE_SIZES)) {
