/**
 * Single source of truth for public storefront SEO copy shared by:
 * - frontend/index.html meta tags
 * - frontend/src/i18n/tk/home.tk.ts hero/CTA overlays
 * - backend/internal/web/prerender_seo_tk.go (kept in sync via scripts/checks/storefront-seo-alignment.py)
 */
export const STOREFRONT_SEO = {
  siteTitle: 'TokenKey - AI API Gateway',
  canonicalOrigin: 'https://tokenkey.dev',
  ogImagePath: '/og-cover.png',
  ogImageUrl: 'https://tokenkey.dev/og-cover.png',
  chinaExport: {
    siteTitle: 'TokenKey - China\'s Leading AI Models, One API',
    metaDescription:
      'Access Seedance, Seedream, Qwen, DeepSeek, GLM and Kimi with one API key. Start with a free trial; no card required.',
    ogDescription:
      'China\'s leading AI models through one API. Build with Seedance, Seedream, Qwen, DeepSeek, GLM and Kimi.',
    twitterDescription:
      'Seedance, Seedream, Qwen, DeepSeek, GLM and Kimi through one API key.',
    canonicalUrl: 'https://global.tokenkey.dev/',
    ogImageUrl: 'https://global.tokenkey.dev/seedance-2-5-official-poster-db3ff793.jpg',
  },
  zh: {
    metaDescription:
      'TokenKey - AI API Gateway. 每一次调用，都是官方品质。文本、图像、视频，一个 Key 全搞定。One key for every AI model, routed direct.',
    ogDescription:
      '每一次调用，都是官方品质。一个 API Key，所有主流 AI 模型。文本、图像、视频。订阅配额，费用可预测。',
    heroTitle: '每一次调用，都是官方品质。',
    heroSubtitle: '一个 API Key，所有主流 AI 模型。文本、图像、视频。订阅配额，费用可预测。',
    freeTrialZh: '开启注册赠额时，首页和价格表会显示当前赠送金额。可使用的 token 数量取决于所选模型及实际请求。',
    ctaDescriptionZh: '足够测试你的真实工作流。只需邮箱，无需信用卡。',
  },
  en: {
    twitterDescription: 'One API key for Claude, GPT, Gemini, and more. Direct-to-vendor quality, predictable pricing.',
    heroTitle: 'One key. Every model. Zero compromises.',
    heroSubtitle:
      'Access Claude, GPT, Gemini, and Alibaba Cloud Qwen through a single API key — routed directly to official endpoints with built-in quota controls.',
    freeTrialEn:
      'When signup credit is available, the current amount is shown on the home and pricing pages. Token usage depends on the model and your requests.',
    ctaDescriptionEn: 'Run your real workload end-to-end. Just an email — no credit card required.',
  },
} as const

export type StorefrontSeo = typeof STOREFRONT_SEO
