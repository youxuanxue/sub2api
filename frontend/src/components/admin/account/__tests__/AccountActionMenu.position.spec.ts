...
const getTop = () => getMenu().style.top === 'auto'
  ? window.innerHeight - Number.parseFloat(getMenu().style.bottom) - getMenu().getBoundingClientRect().height
  : Number.parseFloat(getMenu().style.top)
const getLeft = () => getMenu().style.left === 'auto'
  ? window.innerWidth - Number.parseFloat(getMenu().style.right) - getMenu().getBoundingClientRect().width
  : Number.parseFloat(getMenu().style.left)
...
async function mountMenu(position = new DOMRect(950, 720, 32, 24)) {
...
describe('AccountActionMenu viewport positioning', () => {
  ...
  it('remeasures after reopening on another row', async () => {
    ...
    await wrapper.setProps({ show: true, position: new DOMRect(400, 100, 32, 24) })
    await flushPromises()
