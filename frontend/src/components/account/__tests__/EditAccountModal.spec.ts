...
function buildOpenAIOAuthParentAccount() {
  return { ...buildAccount(), type: 'oauth', credentials: { access_token: 'test-oauth-token' } }
}
...
describe('EditAccountModal', () => {
  it.each(['inherit', 'enabled', 'disabled', 'block'] as const)('persists the single Codex image control in %s mode', async (mode) => {
    ...
    await flushPromises()
    expect(wrapper.findAll('[data-testid^="codex-image-tool-"]')).toHaveLength(4)
    ...
    if (mode === 'enabled' || mode === 'disabled') {
      expect(extra.codex_image_generation_bridge).toBe(mode === 'enabled')
      ...
      if (mode === 'block') expect(extra.codex_image_generation_explicit_tool_policy).toBe('strip')
      else expect(extra).not.toHaveProperty('codex_image_generation_explicit_tool_policy')
    }
    wrapper.unmount()
  })
  beforeEach(() => {
  ...
  it('uses the official xAI base URL when a Grok API-key account omits base_url', async () => {
    ...
    const baseUrlInput = wrapper
      .findAll('input')
      .find(input => (input.element as HTMLInputElement).value === 'https://api.x.ai/v1')
