...
const displayError = computed(() => usageInfo.value?.error || error.value)
const passiveSampleTitle = computed(() => formatDateTime(usageInfo.value?.updated_at))
const passiveSampleLabel = computed(() => {
  const sampledAt = formatDateOnly(usageInfo.value?.updated_at)
  return sampledAt
    ? t('admin.accounts.usageWindow.passiveSampledAt', { time: sampledAt })
