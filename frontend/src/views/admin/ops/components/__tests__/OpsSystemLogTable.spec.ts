...
describe('rolling log retention settings', () => {
  ...
  it('loads and saves separate operations and request retention windows', async () => {
    ...
    fields[1]!.vm.$emit('update:modelValue', 60)
    await saveButton(wrapper).trigger('click')
    await flushPromises()
    expect(mockUpdateRuntimeLogConfig).toHaveBeenCalledWith(expect.objectContaining({ retention_days: 7, request_retention_days: 60 }))
    wrapper.unmount()
  })

  it('rejects zero, excessive, fractional or empty custom days', async () => {
    const wrapper = mountTable()
    await flushPromises()
    const requestField = wrapper.findAllComponents(LogRetentionSelect)[1]!
    for (const value of [0, 91, 1.5, Number.NaN, -1, 3651]) {
