...
import type { Account, AccountListItem, AccountPlatform, AccountSchedulerGroupScore, AccountType, Proxy as AccountProxy, AdminGroup, Group, WindowStats, AccountModelOption, UpstreamBillingProbeSnapshot } from '@/types'
...
const refreshAccountRowMetrics = (refreshTodayStats = true) => {
  const rows = accounts.value.slice()
  if (refreshTodayStats) refreshTodayStatsBatch().catch((error) => {
...
const load = async (options: AccountLoadOptions = {}) => {
  ...
  await baseLoad()
  refreshAccountRowMetrics(options.refreshTodayStats !== false)
}
