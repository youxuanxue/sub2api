export default {
  batchImage: {
    ...
    itemResult: {
      ...
      cancelled: '任务已取消',
      waiting: '等待生成结果',
      emptyImageOutput: '上游返回了结果，但这条没有图片内容。通常是 Google/Vertex 单条生成失败或被安全策略拦截。',
    ...
    create: {
      ...
      selectKeyPlaceholder: '请选择 Google API Key',
      noKeysHint: '当前没有可用于批量生图的 Google API Key。请先创建并绑定已开启批量生图的 Google 分组。',
      model: '模型',
      imageSize: '图片尺寸',
    ...
    guide: {
      title: '批量生图使用说明',
      uiTitle: '当前界面如何使用',
      step1: '1. 选择已开启批量生图的 Google API Key，模型列表会按该 Key 所属分组可用模型展示。',
    ...
    messages: {
      ...
      selectApiKey: '请选择可用的 Google API Key',
      ...
      noCompatibleAccount: '当前密钥所属分组没有可用的批量生图上游账号。请联系管理员检查：该分组是否绑定了可调度的 Google API Key 或 Vertex 服务账号，以及账号是否支持所选模型。',
