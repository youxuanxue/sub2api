export default {
    accounts: {
      ...
      supportedProtocolsHint: '由账号协议探测写入，只读展示；修复上游配置后可在此重新探测。',
      protocolCapabilityEmpty: '未检测到可用文本协议',
      protocolSharedAccounts: '共享于 {count} 个账号',
      protocolLastProbedAt: '最近探测：{time}',
      protocolCapabilityConflict: '同一端点出现冲突证据，协议路由已关闭',
      protocolProbeAction: '重新探测协议',
      protocolProbeRunning: '探测中…',
      protocolProbeUpdated: '协议探测完成，能力已更新',
      protocolProbeUnchanged: '协议探测完成，没有得到可更新能力的新结论',
      protocolProbeInconclusive: '协议探测未得到确定结论，保留现有能力',
      protocolProbeNotApplicable: '该账号当前没有可探测的文本协议',
      protocolProbeFailed: '协议探测失败',
      // Filter options
      allPlatforms: '全部平台',
      ...
      supplierManaged: {
        badge: '供应源托管',
        viewHint: '该账号由供应源创建；在供应源点击「投影账号」会覆盖更新投影字段（如 priority、model_mapping 等）。平时可按普通账号编辑。',
      ...
      duplicatePrompt: '复制「{name}」',
      duplicateCount: '复制数量',
      duplicateCountInvalid: '请输入 1 到 100 之间的整数。',
      duplicateHint: '默认复制 1 个；名称末尾为 -数字时递增，否则从 -1 开始。新账号暂停调度，请重新授权或检查凭据后启用。影子账号复制为需独立授权的普通账号。',
      duplicateBatchSuccess: '已复制 {count} 个账号，均已暂停调度',
      oauthAuthorizationLink: 'OAuth 授权链接',
      ...
      openai: {
        ...
        codexFingerprintModeDesc: '多人共享同一 OAuth 账号时，将各用户的设备/会话标识收敛为账号级恒定值，减少上游可见的设备数和会话数。默认仅收敛设备（installation_id），各 API Key 仍保持独立会话；关闭则原样透传客户端标识，设备+会话曾出现过额度缩水报告。',
      ...
      gemini: {
        ...
        webSessionImportTitle: { initialize: '初始化 Gemini Web Worker 会话', replace: '替换 Gemini Web 浏览器会话' },
        ...
        webSessionImportSuccess: '已替换会话：{count} 个 Cookie（运行时版本 {version}），调度状态不变',
        webSessionInitializeSuccess: '已初始化会话：{count} 个 Cookie（运行时版本 {version}），调度仍关闭，请确认后手动启用',
        tier: {
          label: '账号等级',
        ...
        oauthType: {
          ...
          googleOneDesc: '已弃用的 Gemini CLI 个人订阅通道，仅用于存量账号',
          personalSubscriptionHint:
            '个人 Google AI Pro / Ultra 请改用 Antigravity 平台。Gemini CLI 消费通道已于 2026-06-18 停服。',
          ...
          badges: {
            ...
            individuals: '推荐个人用户',
            legacy: '已弃用',
            useAntigravity: '请改用 Antigravity',
            noGcp: '无需 GCP',
            enterprise: '企业用户',
            noAdmin: '无需管理员配置',
            orgManaged: '组织管理',
