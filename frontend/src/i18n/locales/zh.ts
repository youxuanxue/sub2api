export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      poolModeInfo:
        '启用后：OpenAI / Gemini / Antigravity 上游的 429 / 403 / 401 错误将自动重试，不标记账号限流或错误；Anthropic 上游的 5xx / 403 错误将跳过"连续 3 次失败自动暂禁本账号"的保护，本地账号不被级联拉黑。适用于上游指向另一套 TokenKey / 兼容网关账号池的场景。代价：失去该账号的自动失败保护——只有当你确认上游本身就是会自我轮换的池时再启用。',
