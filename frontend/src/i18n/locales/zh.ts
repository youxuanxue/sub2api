export default {
  ...
  admin: {
    ...
    accounts: {
      ...
      vertexGeminiHint: '这是 Gemini 原生 Vertex 账号，适合文本 Chat；Imagen / Veo 媒体和 GCP 试用消耗请改选「扩展引擎」平台，再选「Vertex AI」渠道类型。',
      ...
      vertexSaJsonUploadHint: '上传、拖入或粘贴 JSON 后会自动读取 project_id；密钥内容仅用于创建/轮换提交。',
      ...
      vertexSaJsonEditHint: '留空则保留当前 Service Account JSON；粘贴新的 JSON 可轮换密钥。',
      ...
      kiroPlatform: {
        ...
        tokenJsonCreateHint:
          '粘贴本机 kiro-auth-token.json（含 accessToken、refreshToken、region、authMethod）。',
        ...
        registrationJsonPlaceholder:
          '粘贴含 clientId、clientSecret 的 ~/.aws/sso/cache/*.json（仅 IdC）',
        registrationJsonCreateHint:
          'IdC 认证必填。粘贴 SSO cache 里含 clientId、clientSecret 的 registration 文件。',
    ...
    ops: {
      ...
      tooltips: {
        ...
        errorTrend: '错误趋势（SLA=platform/provider 故障；客户端过错仅计入分母；上游错误率排除 429/529）。',
        ...
        sla: '服务等级协议：分母为全部终态请求（成功+全部错误），分子仅 platform/provider 故障；客户端过错不拉低 SLA。',
        errors: '错误统计：SLA 错误数为 platform/provider 故障；客户端过错计入总错误但不计入 SLA 分子。',
