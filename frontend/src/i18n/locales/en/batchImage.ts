export default {
  batchImage: {
    ...
    itemResult: {
      ...
      emptyImageOutput: 'The upstream returned a result, but this item has no image content. This usually means a single Google/Vertex generation failed or was blocked by safety policies.',
    ...
    create: {
      ...
      selectKeyPlaceholder: 'Select a Google API key',
      noKeysHint: 'No Google API key is available for batch image generation. Create one and bind it to a Google group with batch image generation enabled first.',
      model: 'Model',
    ...
    guide: {
      ...
      step1: '1. Select a Google API key with batch image generation enabled. The model list shows the models available to that key’s group.',
    ...
    messages: {
      ...
      selectApiKey: 'Select an available Google API key.',
      ...
      noCompatibleAccount: 'No usable upstream batch image account is available for this key’s group. Contact an administrator to check the group’s schedulable Google API key or Vertex service account and model support.',
